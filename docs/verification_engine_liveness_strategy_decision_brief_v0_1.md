...
**Decisions locked:**
- **The verification engine is OURS, portable (ONNX), and the universal default.** A reader's built-in face/liveness (e.g. HN212's hardware anti-spoof) is an OPTIONAL local accelerator — recorded + tier-assessed in the proof, but NEVER the foundation. Any chip-only reader + a webcam is fully served by our engine.
- **Engine placement (tiered):** server-side ONNX is the DEFAULT (any client on any OS just captures + uploads; one engine, consistent, easy to tune — maximally portable). HN212 present → also **consume + tier-assess** its local hardware anti-spoof (attended/kiosk) — rely on it ONLY after the §1a verify-items pass (provenance/cert), not by default. **Capture-binding is required from the FIRST slice for ANY non-HN212-hardware-liveness path** (server-side ONNX on chip-only+webcam too, not just remote): the live-face capture MUST be bound to the session challenge/timestamp — server-side liveness trusts the uploaded frame, so capture-binding is the minimum mitigation against injection/replay. Remote/mobile ADDITIONALLY add client-side active-challenge later.
...
- **Hanel HN212 = the PRIORITY capture adapter (Layer 1), not the platform.** Its built-in `CompareFace` score + `HN212SpoofMode` liveness verdict + `VerifySOD`/`DSCert` passive-auth are ingested as evidence; our engine is what guarantees coverage when the reader lacks them.
- **Hanel SDK analysis (2026-06-24) — what it provides + items to verify:** `VnHn212Reader`/`VnPcscReader`; reads DG1(MRZ)/DG2(face)/DG13(personal)/DG15/SOD with PACE; passive-auth (`DoVerifySOD`/`VerifySOD`/`DSCert`) + active auth (`CheckAACAAuthen`); `CompareFace(chip,cam)→score≥60`; `FaceAntiSpoofMode=HN212SpoofMode{OFF..HIGH}` + `FAIL_SPOOR_FACE`; PaddleOCR. **Verify before relying (when a physical HN212 device is available for hands-on check):** (1) anti-spoof = HN212 hardware vs software, runs off-HN212? certified (iBeta/ISO 30107)? (2) does `VerifySOD` chain to the VN CSCA (real trust anchor) or only check SOD internals? (3) is `CompareFace`/anti-spoof callable standalone (server-side, arbitrary images)? (4) SDK license (per-device/redistribute) + bundled ViewFaceCore/PaddleOCR terms; (5) PACE access — auto-derive from MRZ/CAN scan vs manual `InputAccessCode`; (6) can HN212 outputs be bound to the TagEkyc session challenge/timestamp (anti-replay)? (7) are its match/liveness outputs signed/exported with enough provenance to distinguish the HARDWARE verdict from a software-wrapper verdict? The Hanel SDK is Windows-desktop (PC/SC + DirectShow + USB) → it is a **client-side CaptureAgent/local-agent** (the `integration_channel_strategy` local-agent channel) that submits evidence to TagEkyc `/evidence-results`.
...
| VE-05 | Verify residuals: §1a Hanel HN212 items (when a physical device is available) + telemedicine (Luật KCB 2023/NĐ 96) + controlled-drugs identity bar | Before the relevant slice; BHXH/BHYT DROPPED |
