...
For `Liveness`, the server owns `PayloadHash`: it canonicalizes the final server-normalized decision-basis with `EvidenceCanonicalization.HashCanonical("tip-72-liveness-decision-basis", basis)` and persists that hash. Adapter-supplied `PayloadHash` is optional for Liveness, but if present it must match the server-computed value or the request fails with `LIVENESS_PAYLOAD_HASH_MISMATCH`. `Liveness=Passed` requires a decision basis with score, `method=fixture-liveness` for this fixture/server-path slice, `livenessGrade=passive-2d-only`, server-derived `isLive` (`score >= 0.80`), and trusted same-session live-media capture binding. Missing basis or score/method/grade/artifact id fails with `LIVENESS_DECISION_BASIS_REQUIRED`; malformed score, threshold, result, artifact id/hash, reserved method, or inconsistent `serverDerivedIsLive` fails with `LIVENESS_DECISION_BASIS_MISMATCH`; below threshold downgrades to `ReviewRequired` with `LIVENESS_BELOW_THRESHOLD`; adapter verdict conflict downgrades to `ReviewRequired` with `LIVENESS_VERDICT_MISMATCH`; `silent-face` is reserved for a real engine and fails with `LIVENESS_METHOD_UNEARNED`; `ExternalPreStaged` or unbound live media downgrades with `DIRECT_CLIENT_UPLOAD_UNTRUSTED`. TIP-72 does not run PAD inference, ONNX, image/video decode, or a Silent-Face engine on the server; the real liveness engine is TIP-73. Passive OSS liveness is recorded honestly and is not a certified PAD/ISO-30107 claim.
