...
Fields: `ResultType`, `InputCaptureArtifactIds`, `Result`, `Confidence`, `ReasonCodes`, `RetryReasonCode`, `SanitizedSummaryRef`, `PayloadHash`, `PayloadSignatureStatus`, `EngineName`, `EngineVersion`, `RequestId`, `CorrelationId`, `NfcEvidenceDecisionBasis`, `FaceMatchEvidenceDecisionBasis`, `LivenessEvidenceDecisionBasis`.

`EvidenceResultTypeDto` values: `CaptureQuality`, `DocumentOcr`, `NfcValidation`, `FaceMatch`, `Liveness`, `FingerprintMatch`, `FraudRisk`.

S1 requires non-empty same-session `InputCaptureArtifactIds`, compatible input artifact types, valid confidence when present, non-empty `EngineName` and `EngineVersion`, and a `PayloadHash` with `sha256:` for evidence types that do not have a server-owned decision-basis hash. `FraudRisk` is deferred and rejected by current runtime.
...
For `ResultType=FaceMatch`, TIP-70 adds the optional-on-the-wire but required-for-FaceMatch `FaceMatchEvidenceDecisionBasis` field. It contains the live face artifact id/hash, match score, server threshold/isMatch slots, reference face source (`ChipDg2FromTrustedNfc`, `DocumentImage`, or `SelfieImage`), referenced NFC evidence/artifact ids and hashes, optional referenced NFC `PayloadHash`, live capture-binding object, server/adaptor result fields, engine name/version, sanitized summary label, and extension descriptor. It must contain no raw subject PII, raw biometric bytes, face images, or plaintext document data. Face artifact hashes are biometric-derived/linkable references and are internal-confidential, not public-safe.

For `FaceMatch`, the server owns `PayloadHash`: it canonicalizes the final server-normalized decision-basis with `EvidenceCanonicalization.HashCanonical("tip-70-face-match-decision-basis", basis)` and persists that hash. Adapter-supplied `PayloadHash` is optional for FaceMatch, but if present it must match the server-computed value or the request fails with `FACE_MATCH_PAYLOAD_HASH_MISMATCH`. `FaceMatch=Passed` requires a decision basis with a match score, server threshold pass, same-session live face capture binding, and `referenceFaceSource = ChipDg2FromTrustedNfc` referencing a same-session `NfcValidation` evidence with `Result == Passed`, a `PayloadHash`, and matching NFC input artifact hash. Missing basis fails with `FACE_MATCH_DECISION_BASIS_REQUIRED`; adapter `isMatch` conflict fails with `FACE_MATCH_DECISION_BASIS_MISMATCH`; below threshold downgrades to `ReviewRequired` with `FACE_MATCH_BELOW_THRESHOLD`; non-chip references downgrade with `FACE_MATCH_REFERENCE_NOT_PRODUCTION_GRADE`; missing/cross-session/non-passed NFC references downgrade with `FACE_MATCH_REFERENCE_NFC_NOT_TRUSTED`; `ExternalPreStaged` or unbound live face capture downgrades with `DIRECT_CLIENT_UPLOAD_UNTRUSTED`. TIP-70 does not re-run a face engine or liveness/PAD and does not re-read NFC chip flags; the referenced NFC `Passed` result is the TIP-69 append-time guarantee.

For `ResultType=Liveness`, TIP-72 adds the optional-on-the-wire but required-for-Liveness `LivenessEvidenceDecisionBasis` field. It contains the live media artifact id/hash, normalized `livenessScore`, `adapterRequestedVerdict` (`live`, `spoof`, or `uncertain`, audit/cross-check only), `method`, `livenessGrade`, threshold slot, live capture-binding object, `serverDerivedIsLive`, server/adaptor result fields, sanitized summary label, and extension descriptor. It must contain no raw subject PII, raw biometric bytes, video bytes, still frames, or plaintext document data. `EngineName`/`EngineVersion` remain top-level request fields only; they are not duplicated in the liveness basis.

For `Liveness`, the server owns `PayloadHash`: it canonicalizes the final server-normalized decision-basis with `EvidenceCanonicalization.HashCanonical("tip-72-liveness-decision-basis", basis)` and persists that hash. Adapter-supplied `PayloadHash` is optional for Liveness, but if present it must match the server-computed value or the request fails with `LIVENESS_PAYLOAD_HASH_MISMATCH`. `Liveness=Passed` requires a decision basis with score, `livenessGrade=passive-2d-only`, server-derived `isLive` (`score >= 0.80`), and trusted same-session live-media capture binding. TIP-73 earns the real `silent-face` method only through an exact server-side allowlist consistency check: `method=silent-face` is accepted iff top-level `EngineName=silent-face-minifasnet` and `EngineVersion=minifasnet-v2+onnxruntime-1.20.1`; `method=fixture-liveness` is accepted iff the top-level engine is the fixture liveness engine. Cross-cases fail closed without new error codes: `silent-face` with missing/fixture/unknown/typo engine fails with `LIVENESS_METHOD_UNEARNED`, while `fixture-liveness` with the real engine fails with `LIVENESS_DECISION_BASIS_MISMATCH`. Missing basis or score/method/grade/artifact id fails with `LIVENESS_DECISION_BASIS_REQUIRED`; malformed score, threshold, result, artifact id/hash, unsupported method, or inconsistent `serverDerivedIsLive` fails with `LIVENESS_DECISION_BASIS_MISMATCH`; below threshold downgrades to `ReviewRequired` with `LIVENESS_BELOW_THRESHOLD`; adapter verdict conflict downgrades to `ReviewRequired` with `LIVENESS_VERDICT_MISMATCH`; `ExternalPreStaged` or unbound live media downgrades with `DIRECT_CLIENT_UPLOAD_UNTRUSTED`. The server still does not run PAD inference, ONNX, image/video decode, or a Silent-Face engine; TIP-73's engine is agent-side. Passive OSS liveness is recorded honestly and is not a certified PAD/ISO-30107 claim.

TIP-71 pins the capture-binding challenge hash as an interop contract for sibling capture agents. Compute it as `HashCanonical("tip-69-capture-session-challenge", { "challenge": <raw session challenge>, "sessionId": <N-format session id> })`, where `HashCanonical(label, value)` is `sha256:` + lower-hex SHA-256 of `label + "\n" + RFC8785/JCS(value)` using UTF-8. Golden vector: `sessionId=0123456789abcdef0123456789abcdef`, `challenge=tip-71-fixed-challenge` => `sha256:c26dfddcec7dd5579c2b0787fb33a0eca6dc9906dce1e849ae7bc54cb0c3a43b`.
