...
Fields: `ResultType`, `InputCaptureArtifactIds`, `Result`, `Confidence`, `ReasonCodes`, `RetryReasonCode`, `SanitizedSummaryRef`, `PayloadHash`, `PayloadSignatureStatus`, `EngineName`, `EngineVersion`, `RequestId`, `CorrelationId`, `NfcEvidenceDecisionBasis`, `FaceMatchEvidenceDecisionBasis`.

`EvidenceResultTypeDto` values: `CaptureQuality`, `DocumentOcr`, `NfcValidation`, `FaceMatch`, `Liveness`, `FingerprintMatch`, `FraudRisk`.

S1 requires non-empty same-session `InputCaptureArtifactIds`, compatible input artifact types, valid confidence when present, non-empty `EngineName` and `EngineVersion`, and a `PayloadHash` with `sha256:` for non-NFC/non-FaceMatch evidence. `FraudRisk` is deferred and rejected by current runtime.
