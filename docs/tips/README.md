...
| TIP-67B | `tip_67b_neutral_verifiable_proof/` | [`tip_67b_planning_brief_v0_1.md`](tip_67b_neutral_verifiable_proof/tip_67b_planning_brief_v0_1.md) - draft planning brief (v0.5, **BLOCKED on 67A** — kickoff held until 67A freezes the challenge/profile contract); **neutral verifiable eKYC proof + verification view**, depends on TIP-67A — sign a neutral proof claim `{sessionId, identityRef, result, assuranceLevel, requiredChecks/completedChecks/evidenceResultTypes/engine name+version/achievedAssurance, timestamp, challenge(echoed), signedManifestHash, resultHash}` (E-Q02, self-contained), expose a verification view + `publicKeyJwk` + echoed challenge, publish the fail-closed verifier contract (verify against an **out-of-band trust anchor** — pinned kid + `publicKeyFingerprint`, not the embedded JWK) + a reference consumer-verification test incl. a forged-JWS+attacker-JWK negative test (no runtime endpoint); the client verifies + does its OWN binding `H(challenge ‖ document_hash ‖ signed resultHash)` (eKYC neutral, never binds the document/transaction). Carries forward the sound parts of the superseded TIP-67 v0.3 (ES256 JWS, fail-closed, no-leak sanitization, trace matrix reframed challenge-based); full canonical-preimage reproduction deferred (Tier-2, exposable on demand); dev-key only (prod HSM/KMS/JWKS/CA/RFC-3161/webhook = P0; decision-basis binding = P1) |
