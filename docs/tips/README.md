...
- TIP-73 (`tip_73_agent_side_liveness/`, brief v0.3) built as the **agent-side Silent-Face/MiniFASNet-V2 liveness engine** feeding the existing TIP-72 server Liveness evidence contract. Server delta is intentionally tiny: `silent-face` is accepted only when `method=silent-face`, `livenessGrade=passive-2d-only`, and top-level `EngineName=silent-face-minifasnet` + `EngineVersion=minifasnet-v2+onnxruntime-1.20.1` match the explicit allowlist; fixture remains `fixture-liveness` with its fixture engine. Cross-cases fail closed (`LIVENESS_METHOD_UNEARNED` / `LIVENESS_DECISION_BASIS_MISMATCH`) with no new error codes, no threshold/assurance/canonicalization/manifest shape change, and engine identity remains proof-bound through `evidenceEngines -> resultHash`/ES256-JWS rather than the Liveness basis `PayloadHash` or manifest hash. CaptureAgent adds the real/fake `ILivenessScoreSource`, model-card-pinned MiniFASNet preprocessing (single face detect -> canonical 2.7x bbox-center crop -> 80x80 BGR -> `/255` -> NCHW, output `[live,print,replay]` index 0), fail-closed wrappers, no-raw disposal, LivenessMedia-only request mapping, license/provenance packet, deterministic tests, and opt-in real-ONNX orientation smoke. Local model/media are gitignored and not committed; passive OSS liveness remains not certified PAD/ISO-30107 and `0.80` remains carried-not-calibrated. Baselines: server `2cd9c2d`, agent `4f8fa6a`.

### v1.33 - TIP-72 built (server-side Liveness evidence path; engine deferred to TIP-73)

- TIP-72 (`tip_72_liveness_evidence/`, brief v0.5) built as the **server-side Liveness evidence path only**: additive `LivenessEvidenceDecisionBasis` -> server-owned `PayloadHash` (`HashCanonical("tip-72-liveness-decision-basis", basis)`) -> manifest/proof. The server derives `isLive` from score >= threshold (0.80), not adapter verdict; `adapterRequestedVerdict` is audit/cross-check only. `Passed` requires server-derived live + trusted same-session live-media binding; bare/malformed/hash-mismatch fail closed (`LIVENESS_DECISION_BASIS_REQUIRED` / `LIVENESS_DECISION_BASIS_MISMATCH` / `LIVENESS_PAYLOAD_HASH_MISMATCH`), below-threshold and verdict-conflict coerce `ReviewRequired` (`LIVENESS_BELOW_THRESHOLD` / `LIVENESS_VERDICT_MISMATCH`), and unbound/`ExternalPreStaged` media coerces with `DIRECT_CLIENT_UPLOAD_UNTRUSTED`. Earned-method rule is locked: this server slice accepts `method=fixture-liveness` + `livenessGrade=passive-2d-only`; `silent-face` is reserved for a real engine and rejected (`LIVENESS_METHOD_UNEARNED`). No server PAD inference/ONNX/decode, no manifest/proof shape change, no `CalculateAssuranceLevel` change; passive OSS liveness is recorded honestly and is **not certified PAD/ISO-30107**. Real Silent-Face/MiniFAS engine = TIP-73 agent-engine.

### v1.32 - TIP-71 BUILT (agent ViewFaceCore engine, pts5 fix) — engine-verified on web photos, NOT production-ready

- TIP-71 agent (`D:\Task\Remote Signing\TagEkyc.CaptureAgent`, its OWN git repo — sibling solution outside `TagEkyc.sln`, refs Contracts DTOs only, NO ViewFaceCore dep in server) built through brief v0.5 (5 GPT+Codex review rounds) + adversarial CODE verification. Server-side additions: `lld_03` challengeHash formula + golden vector; `Tip05` golden assertion + PcAgent happy-path. **Adversarial verify caught TWO defects the green tests hid:** (1) the engine was first shipped as a **marker** (no real ViewFaceCore call, stamped an unearned engineName) → remediated to a REAL pipeline (decode→`FaceDetector.Detect`→`FaceLandmarker.Mark`→`FaceRecognizer.Extract`→`Compare`), earned-engineName invariant (fixture path = `fixture-facematch`), engine-failure fail-closed (no-face/multi-face/decode/timeout → never Passed), real no-raw zeroing; (2) **a SeetaFace landmark-model bug**: `FaceRecognizer`(Normal) aligns its crop with **5-point** landmarks (`FaceLandmarker` Light/pts5) — the build's `pts68`/Normal landmarker MIS-ALIGNED the crop → same/different similarity gap collapsed 0.39→0.03 (a DIFFERENT person false-positived at 0.836 > server 0.80). A controlled pts5-vs-pts68 experiment (same images, isolate the landmark variable) confirmed it; production default flipped to **pts5/Light** (web-photo result: same 0.895 / different 0.505 / gap 0.39; opt-in test now asserts `different < 0.62` + `gap > 0.20`). The govern-loop had MISDIAGNOSED this as "threshold-0.80 calibration debt" twice. challengeHash interop is byte-identical to the server (independent canonicalizer + golden, no `TagEkyc.Application` ref). **NOT production-ready** — debt: threshold operating-point calibration (FAR/FRR on a real pair set; 0.80 conservative vs SeetaFace-native 0.62 = TIP-70 follow-on), engine-adequacy on REAL DG2-chip(low-res)-vs-live-selfie + dlib comparison, license-packet completeness, cross-principal binding. Native ViewFaceCore inference is opt-in (Homeowner-run locally on Windows; CI runs the wrapper via a fake pipeline). Baseline `d6e1f9b`; agent repo committed separately.

### v1.31 - TIP-71 (agent-side FaceMatch reference) opened — READY_FOR_GPT_REVIEW

- Added TIP-71 (`tip_71_agent_side_facematch/`, planning brief v0.1). A DIFFERENT slice type — a **client CaptureAgent reference implementation, NOT a TagEkyc-server change**. Locked decisions: engine = **ViewFaceCore/SeetaFace6** (.NET-native, commercial-clean; ArcFace/InsightFace pretrained = OFF-LIMITS due to the MS1M/MS-Celeb/Glint360K/VGGFace2/CASIA non-commercial-dataset trap — model-license research 2026-06-27); placement = **agent-side** (FaceMatch/Liveness run in the trusted CaptureAgent/PcAgent, NOT on the server; server stays biometric-free + engine-free; **agent-side inference is execution placement, NOT a trust boundary**); **no-raw-server retention KEPT** as the foundational posture (raw-biometric retention reconsideration = KEEP_NO_RAW_DEFAULT; the P0 raw control RELOCATES to the agent — in-memory only, no persist/log/crash-dump, secure disposal, no real faces in repo/tests); EvidenceVault/raw-packet = optional follow-on (ART-009/ART-001 + TIP-43/44/45/46), server-side transient inference deferred to a future hosted-web/mobile slice. The agent submits NFC `Passed` + FaceMatch evidence through the EXISTING TIP-69/70 server contract (server unchanged). License-evidence packet required before production ship; dlib = documented fallback. Baseline `d6e1f9b`. Pending GPT/Codex review.

### v1.30 - TIP-70 (FaceMatch evidence path) built + mutation-tested + committed

- TIP-70 (`tip_70_face_match_evidence/`, brief v0.4) = verification-engine slice 2: server-side proof-bound FaceMatch evidence path, mirrors TIP-69. `FaceMatchEvidenceDecisionBasis` → server-OWNED `PayloadHash` (`HashCanonical("tip-70-face-match-decision-basis", basis)`) → manifest/proof. **Core security (the P1 the reviews caught): reference-face source LOCKED** — production-grade FaceMatch requires `referenceFaceSource=ChipDg2FromTrustedNfc` bound to a SAME-SESSION `NfcValidation` with `Result==Passed` (TIP-69 append-time fail-closed IS the guarantee; flags not re-read — they aren't persisted queryably) whose input artifact is the referenced NfcReadArtifact; non-chip / missing / cross-session / non-Passed reference → coerced `ReviewRequired` (`FACE_MATCH_REFERENCE_*`). Stops "image-similarity masquerading as identity". Server owns `isMatch` (= score ≥ 0.80 server const) + threshold; below → ReviewRequired + `FACE_MATCH_BELOW_THRESHOLD`; adapter isMatch/hash conflict → MISMATCH. Role-aware live-binding (CAPTURE_BOUND_TO_SESSION on the live selfie); score adapter-declared, `IFaceMatchEngine` NOT wired (real engine = TIP-71). 3 GPT+Codex review rounds. **Contractor adversarial CODE verify used MUTATION TESTING** (broke each guard → confirmed tests go red) — strongest verification tier so far; reference-trust tests build REAL Passed/non-Passed/cross-session NFC. No manifest/proof SHAPE change; `CalculateAssuranceLevel` unchanged; legacy golden byte-identical. Committed `5626e9d` (+ `d6e1f9b` sanitized-label hardening shared with TIP-69). Baseline `448a05e`.

### v1.29 - TIP-69 (NFC chip-evidence decomposition) built + committed; + completion-level + sanitized-label hardening

- TIP-69 (`tip_69_verification_engine_slice1/`, brief v0.7) = verification-engine slice 1, framed as the **Verification-Extension seam**: server-side proof-bound NFC chip-evidence decomposition. Canonical `NfcEvidenceDecisionBasis` (subject-PII-free) → server-OWNED `PayloadHash` (`HashCanonical("tip-69-nfc-evidence-decision-basis", basis)`) → manifest/proof (**the binding surface is PayloadHash, NOT ReasonCodes/SanitizedSummaryRef**). Server-authoritative fail-closed: bare `NfcValidation=Passed` → `NFC_EVIDENCE_DECOMPOSITION_REQUIRED`; adapter hash mismatch → `NFC_PAYLOAD_HASH_MISMATCH`; ExternalPreStaged / un-bound / negative-binding → coerced `ReviewRequired` + `DIRECT_CLIENT_UPLOAD_UNTRUSTED`; serverDecisionResult normalized before hashing (payload ≡ persisted Result); `CAPTURE_BOUND_TO_SESSION` server-stamped iff `IsCaptureBindingTrusted`. `IFaceMatchEngine` port + deterministic fixture (seam only). 5 GPT+Codex review rounds (round-1 caught a P1: VE-07 flags would've shipped cosmetic-not-proof-bound). Adversarial CODE verify. No manifest/proof SHAPE change; `CalculateAssuranceLevel` unchanged; legacy golden byte-identical. Committed `e1d9e29`; + completion-level test (ExternalPreStaged NFC cannot complete as Passed/High; GPT post-push CLEAR) `448a05e`; + `SanitizedSummaryLabel` fail-closed validation (NFC + FaceMatch basis-level; GPT review P2) `d6e1f9b`. Baseline `140ea29` (verification-engine strategy v1.0).

### v1.28 - TIP-68 BUILT + spot-check ACCEPT (2 rounds, SoftHSM e2e); ready for commit

- TIP-68 (P0 prod-crypto Slice A — production HSM/PKCS#11 signing) built by Codex; Contractor adversarial spot-check on CODE = ACCEPT after 2 rounds. **Round 1 caught a CRITICAL**: build was 9/9 green but NO test opened a token — the PKCS#11 sign path had ZERO runtime coverage (the green count masked it; provisioning `.ps1` orphaned). NOT accepted → forced a real SoftHSM end-to-end. **Round 2** (post-remediation): SoftHSM2 2.5.0 installed; 5 new e2e tests genuinely OPEN a token (on-token keygen `CKA_TOKEN`/`SENSITIVE`/non-`EXTRACTABLE`, hard-fail not skip), sign on-token via `CKM_ECDSA`, reference-verify the JWS under the token-exported public JWK + pinned fingerprint, fail-closed self-test reached via a REAL public/private key mismatch, concurrent-sign. `dotnet test` **190/190** (Tip68 14/14, SoftHSM e2e 5/5, skipped 0); golden `manifestBodyHash`/`packageHash`/`manifestHash` byte-identical. Closeout v0.1 (playbook). Pre-commit cleanup: `SOFTHSM2_MODULE` canonical fixture override + `PATH` reverted on dispose. Strongest "test-green ≠ correct" vindication so far. Ready for Codex commit by allowlist. Deferred: prod hardware HSM provisioning (P0-infra), SoftHSM-in-CI (P1), CA/TSA/qualified (legal milestone), JWKS (A2).

### v1.27 - TIP-68 ready to dispatch after deep doc/code review

- TIP-68 (P0 prod-crypto Slice A) is indexed as **READY_TO_DISPATCH** after review cleanup. Current artifacts: planning brief v0.8 and kickoff v0.6, with kickoff §5 as the authoritative DoD. Scope is PKCS#11 HSM signing custody behind the existing `IEvidenceSigner`, SoftHSM dev/CI on the same adapter path, eager env-gated fail-closed DI, and key rotation with consumer trust-anchor history. Runtime signs only the neutral eKYC proof; `manifestHash` is bound inside the proof claim, no second signature/envelope/field is added, and golden hash-chain values remain byte-identical. JWKS stays deferred to Slice A2. CA cert, RFC-3161 TSA, qualified non-repudiation, webhook signing, document/transaction binding, and runtime verifier engine remain out of scope. Root README was clarified so TagEkyc may sign its own eKYC proof/evidence envelope while still not performing document/transaction/qualified signing for SignFlow.

### v1.26 - TIP-68 (P0 prod-crypto Slice A) drafted; superseded by v1.27

- P0 prod-crypto track opened after 67B committed. Decision brief `docs/p0_prod_crypto_decomposition_and_decision_brief_v0_1.md`: `IEvidenceSigner` is a clean single seam → prod = adapter + env-gated DI, no architecture change. **D-P0-1 DECIDED** = on-prem HSM/PKCS#11 (dev/CI = SoftHSM). **D-P0-2 RESOLVED by legal research** (Luật GDĐT 20/2023 Điều 10/11 + NĐ 23/2025 which replaced the dead NĐ 130/2018): trusted timestamp (RFC-3161) / qualified signature are NOT required on the eKYC record for evidentiary value (integrity + originator + complete-form retrieval suffice) → CA cert / TSA / qualified non-repudiation DEFERRED behind a legal milestone. TIP-68 (Slice A) initial draft: brief v0.2 + kickoff v0.1 — PKCS#11 HSM signing adapter behind `IEvidenceSigner` + env-gated fail-closed DI + key rotation (historical packages verify via TIP-67B persisted sign-time key; no JWKS). JWKS deferred to Slice A2; CA-forward-compat invariants pinned (additive scheme, fail-closed-on-unknown). Golden hash chain byte-identical (signing downstream). Superseded by v1.27 review-cleaned READY_TO_DISPATCH entry.

### v1.25 - TIP-67B spot-check ACCEPT (2 rounds, closeout v0.2); ready for commit

- TIP-67B Contractor adversarial spot-check on CODE = CLEAN (2 rounds). Round 1: 8/8 load-bearing properties verified (sign-time key binding — view serves persisted sign-time key, NOT current/rotated signer; negative matrix reaches real ECDSA verify + fingerprint pin + mirror cross-check + resultHash recompute; identityRef always-hash no raw PII; deterministic resultHash; provider-neutral no concrete-signer cast; golden byte-identical; eKYC neutral). One LOW gap raised: no isolated `VerifyData()==false`. Round 2 (post-patch) verified on CODE: **Fix A** = signature-byte-tamper test (flips `parts[2]` only, intact fingerprint/kid → reaches ECDSA verify-false, not a pre-check); **Fix B** = `signflow_integration_contract` expanded to a self-contained verifier algorithm (SignFlow implements from the doc alone). `dotnet test` **176/176**; golden `manifestBodyHash`/`packageHash`/`manifestHash` byte-identical. Closeout v0.2 (full playbook structure). Ready for Codex commit by allowlist (NOT pushed). **SignFlow core integration loop complete at dev-level** (T2-1 JCS + T2-2 signing + 67A neutral profile + 67B neutral verifiable proof); prod HSM/KMS/JWKS/CA/RFC-3161 + decision-basis binding = P0/P1 debt.

### v1.24 - TIP-67B built; spot-check pending

- TIP-67B built by Codex on baseline `72a7b41` (175/175). Adds neutral self-contained signed proof (`neutral-proof-v1`) via dedicated `EvidenceProofSignatureRequest`, `resultHash`, hashed `identityRef`, sorted checks, ordered `evidenceEngines`, sign-time `publicKeyJwk`/`publicKeyFingerprint`, and `GET /api/ekyc/evidence-packages/{id}/verification-view`. Summary route unchanged; no document/transaction binding in TagEkyc; no runtime verifier endpoint; no prod HSM/KMS/JWKS/CA/webhook. Golden `manifestBodyHash`/`packageHash`/`manifestHash` byte-identical.

### v1.23 - TIP-67A built + spot-check ACCEPT (closeout v0.1); 67B unblocked

- TIP-67A (eKYC neutrality) built by Codex (171/171). Contractor adversarial spot-check on CODE = CLEAN: profile-ONLY wire converter (global converter unchanged), opaque `Challenge` string, request field-key compat + `CONFLICTING_CHALLENGE_FIELDS`, echo on the 3 response DTOs (not in hash), policy/error-codes neutral (zero surviving transaction interpretation in the core), golden vectors byte-identical. ONE **disclosed scope exception** accepted (Homeowner): a carried TIP-66 `signedAt` microsecond-precision fix (Postgres `timestamptz` roundtrip; no hash-graph change) — recorded in the closeout's dedicated section + an `lld_01` note. Closeout v0.1 written (two-group file list, playbook structure). Ready for Codex commit; the opaque-challenge/profile contract is FROZEN → TIP-67B unblocked.

### v1.22 - eKYC neutrality course-correction; TIP-67 superseded → split into 67A + 67B

- A forgotten neutral-eKYC vision (`Codex_SignFlow/docs/Idie/vision_neutral_ekyc_service_v0_1.md`, written during SignFlow work, never reviewed when TagEkyc was built) was reconciled against TagEkyc and revealed a drift: TagEkyc absorbed SignFlow's "transaction" concept (`TRANSACTION_BOUND_EKYC_PROFILE` + `externalTransactionId` + mandatory binding) into the eKYC core, violating neutrality (binding is the client's job; the challenge must be opaque). Recorded the decision in `docs/adr/ekyc_neutrality_decision_v0_1.md` (Accepted) + registered the drift as P1 debt. **TIP-67 v0.3 (transaction-bound verifiable evidence) is SUPERSEDED** and removed; it is split (to avoid one TIP both refactoring shipped architecture and opening a public verification surface): **TIP-67A** = neutralize the profile (opaque challenge, echo, compat) — a shipped-code refactor; **TIP-67B** = neutral verifiable proof + verification view (depends on 67A's frozen contract). Build is sequential (67A → 67B); 67B's kickoff is held until 67A freezes the challenge/profile shape (67A is 67B's input). E-Q05: SignFlow's `IEkycEngine` is a per-provider abstraction; a TagEkyc adapter is feasible (contract adaptable to the neutral flow); TagEkyc is its own provider (its own flow); SignFlow writes a TagEkyc-specific adapter (confirm the exact delta with a SignFlow-side audit).

### v1.21 - TIP-67 transaction-bound verifiable evidence (SignFlow integration MVP) opened

- Added TIP-67 (`tip_67_transaction_bound_verifiable_evidence/`, planning brief + kickoff **v0.3** after Homeowner re-review + a repo sweep that named the "Evidence Binding / Verifier Surface Gap" class — *field exists ≠ in hash graph ≠ signed ≠ consumer-verifiable* — and added a mandatory evidence-binding **trace rule + acceptance matrix** as a standing gate) following a SignFlow-integration-surface investigation (TagEkyc ~70% ready). Closes **5 integration gaps**: (1) transaction context (`externalTransactionId`/`bindingNonceHash`/`externalSessionId`) is NOT in the hashed/signed evidence (verified — anti-replay gap) → bind into the **manifest body for TRANSACTION_BOUND profile only** (standard golden byte-identical); (2) public DTO exposes signature STATUS only — no JWS/envelope/public key; (3) **key correction (Option B)** — the public summary lacks the PREIMAGE (omits `auditEventRefs`/`resultRef`, truncates evidence refs dropping `PayloadHash`) to reproduce `manifestHash`, so exposing JWS+JWK alone only proves "the JWS signs the returned hash" → add a dedicated **`EvidencePackageVerificationViewDto`** exposing the full canonical manifest-body preimage so the consumer reproduces `manifestBodyHash`/`packageHash`/`manifestHash` via JCS, then verifies the JWS; (4) completion notification pinned **notification-only** (verify via package retrieval); (5) publish the fail-closed verifier contract (incl. STANDARD-as-TRANSACTION_BOUND reject) + a reference consumer-verification test using ONLY the view (no runtime verifier endpoint). Sanitization intact (no VaultRef/raw/PII/private-key). Dev-level; production HSM/KMS + JWKS rotation + CA cert + RFC 3161 timestamp + webhook signing remain P0; decision-basis binding remains P1 (package not yet legal-complete). Baseline `e8cf836`. Pending GPT/Codex review.

### v1.20 - TIP-66 built, spot-checked, patched + accepted (closeout v0.1)

- TIP-66 was built by Codex (167/167) and patched after the Contractor adversarial spot-check (verified on CODE). Core confirmed: golden `manifestHash`/`packageHash`/`manifestBodyHash` byte-identical (signing downstream/additive), real ECDSA P-256, envelope single-source on the manifest row (package row untouched), mapper fail-closed both directions, no key material committed, public DTO exposes no JWS, lld_01/lld_03 honest. Patch closed two gaps: (1) added `VerifySignatureOnly` + `ReplaceSignature` tests so the cryptographic check itself is proven to reject tampered signed content and tampered signature bytes (the prior echo-compare short-circuited before `VerifyData`); (2) bound `signedAt` into the signed claim (signing time now tamper-evident). Spot-check round 3 = ACCEPT; closeout v0.1 written. Not yet committed (Codex commit+push pending). T2-2 resolved at dev level; production HSM/KMS + CA cert + RFC 3161 trusted timestamp + non-repudiation remain P0 debt.

### v1.19 - TIP-66 evidence-package signing opened

- Added TIP-66 (`tip_66_evidence_package_signing/`, planning brief + kickoff **v0.2**) per the Homeowner T2-2 decision + the 2026-06-22 algorithm/backend/verify-scope choices: ONE `IEvidenceSigner` contract (`SignAsync(EvidenceSignatureRequest)`→`EvidenceSignatureEnvelope`) + a local-key/P12 **ES256** dev adapter producing an **attached compact JWS over a stable signed claim object** `{purpose, signedManifestHash, packageId, packageVersion, canonicalizationScheme, hashAlgorithm, signedAt}` (binds context, not a bare hash); self-describing envelope (`signatureFormat=JWS`/`signatureScheme=jws-es256-v1`/`signatureAlgorithm=ES256`/`keyId`/`signedAt`/`signatureValue` + JWS header `alg`/`kid`) so a future algorithm/backend swap is non-breaking; persisted on the authoritative **manifest row only** (single source, not the package row) + internal-exposed; verify = round-trip + full negative matrix test (no runtime endpoint, no dead-code engine — TIP-65 lesson). Scope = EvidencePackage signature only (payload/webhook signature out); legacy packages stay `PlaceholderUnverified`; signing is downstream/additive and must NOT change `manifestHash` (golden vectors byte-identical). Resolves T2-2 at dev level; prod HSM/KMS + CA cert + non-repudiation stay P0 debt. GPT/Codex review accepted (only P3 index cleanup) — **READY TO DISPATCH**. (Supersedes the earlier TIP-65 historical "SoftHSM" wording for TIP-66: dev backend is local non-production key/P12 only — no SoftHSM/PKCS#11/HSM/KMS.)
...
- Added TIP-65 RFC 8785 JCS Canonicalization planning brief + kickoff (draft) to the index.
- Recorded the Homeowner T2 decisions (2026-06-21): T2-1 adopt RFC 8785 JCS (versioned, record scheme); T2-2 abstract `IEvidenceSigner` (dev = SoftHSM, prod = HSM/KMS, format = JWS) deferred to TIP-66 after TIP-65.
- TIP-65 is an implementation/Tier-2 (EBS-01) TIP: replace Web-JSON canonicalization with RFC 8785 JCS in `HashCanonical`/`DeterministicGuid`, pin deterministic timestamp (`...fffffffZ` full-precision)/guid formatting, record three durable metadata fields (`packageVersion`/`canonicalizationScheme`/`hashAlgorithm`) — hashed + persisted (EF rows/mapper + minimal migration) + exposed via the internal manifest — and prove portability via an independent cross-verification test (a non-app RFC 8785 implementation reproduces the app hashes) + RFC 8785 conformance + legacy/fail-closed/future-field tests; updates `lld_01` and marks T2-1 resolved. Public summary + lld_02/03 untouched.
- Baseline `951694a docs: close TIP-64 ...`. JCS must precede signing (sign over the stable canonical hash). Scope: canonicalization code, tests, `lld_01`, the three metadata fields (`packageVersion`/`canonicalizationScheme`/`hashAlgorithm`) on the domain record + internal manifest DTO + EF rows/mapper, and the minimal migration; public BusinessConsumer summary + `lld_02`/`lld_03` remain untouched; no signing, no hash-chain-topology change, no API/business change beyond the three metadata fields.
...
| TIP-65 | `tip_65_rfc8785_jcs_canonicalization/` | [`tip_65_planning_brief_v0_1.md`](tip_65_rfc8785_jcs_canonicalization/tip_65_planning_brief_v0_1.md) - draft implementation/Tier-2 (EBS-01) planning brief (v0.3), [`tip_65_kickoff_v0_1.md`](tip_65_rfc8785_jcs_canonicalization/tip_65_kickoff_v0_1.md) - draft dispatch-ready kickoff (v0.3); baseline `951694a docs: close TIP-64 S1 evidence-integrity consolidation`; pending GPT/Codex convergence before build; resolves Homeowner T2-1 decision by replacing Web-JSON canonicalization with RFC 8785 JCS in `HashCanonical`/`DeterministicGuid`, pinning deterministic timestamp (`...fffffffZ` full-precision)/guid formatting, recording durable+retrievable metadata (`packageVersion=evidence-package-v2`/`canonicalizationScheme=rfc8785-jcs-v1`/`hashAlgorithm=sha256`, hashed+persisted+exposed), versioning DeterministicGuid, and documenting the legacy/verifier fail-closed rule (rule+tests, no built verifier engine); proven by an independent cross-verification test (a 2nd RFC 8785 implementation reproduces the app hashes) + RFC 8785 conformance + legacy/fail-closed/future-field tests; updates `lld_01` + marks T2-1 resolved; authorized minimal additive delta = the three metadata fields persisted (EF rows/mapper + minimal migration) + exposed via the INTERNAL manifest DTO; public BusinessConsumer summary + `lld_02`/`lld_03` untouched (public exposure deferred); no signing (T2-2 = TIP-66), no hash-chain-topology change, no API/business/persistence change beyond the three metadata fields |
| TIP-66 | `tip_66_evidence_package_signing/` | [`tip_66_planning_brief_v0_1.md`](tip_66_evidence_package_signing/tip_66_planning_brief_v0_1.md) - draft implementation/Tier-2 (EBS-07) planning brief (v0.2), [`tip_66_kickoff_v0_1.md`](tip_66_evidence_package_signing/tip_66_kickoff_v0_1.md) - draft dispatch-ready kickoff (v0.2), [`tip_66_closeout_v0_1.md`](tip_66_evidence_package_signing/tip_66_closeout_v0_1.md) - closeout (v0.1); baseline `a98f278`; BUILT + spot-check ACCEPT after one patch round, ready for Codex commit+push; resolves T2-2 at dev level by adding `IEvidenceSigner` + `EvidenceSignatureRequest`/`EvidenceSignatureEnvelope` + a local non-production key/P12 **ES256** adapter producing an **attached compact JWS over a stable signed claim object** `{purpose, signedManifestHash, packageId, packageVersion, canonicalizationScheme, hashAlgorithm, signedAt}` (binds context, not a bare hash); the envelope `signatureFormat=JWS`/`signatureScheme=jws-es256-v1`/`signatureAlgorithm=ES256`/`keyId`/`signedAt`/`signatureValue` is persisted on the authoritative **manifest** row (single source, not the package row) + internal-exposed only; status `Signed` added while legacy packages remain `PlaceholderUnverified`; tests cover round-trip, wrong-key, header-alg vs recorded-alg mismatch, unknown alg/scheme fail-closed, tamper, claim-vs-row mismatch, legacy placeholder, and `manifestHash`/`packageHash`/`manifestBodyHash` golden vectors byte-identical; RSA/HSM/KMS = future adapters behind the SAME contract; no runtime verify endpoint, no public JWS exposure, no payload/webhook signing, no SoftHSM/PKCS#11/HSM/KMS/Key Vault, no CA/legal/non-repudiation claim; updates `lld_01` + minimal `lld_03` enum note, marks T2-2 dev-resolved |
| TIP-67A | `tip_67a_ekyc_neutrality_opaque_challenge/` | [`tip_67a_planning_brief_v0_1.md`](tip_67a_ekyc_neutrality_opaque_challenge/tip_67a_planning_brief_v0_1.md) - draft planning brief (v0.6), [`tip_67a_kickoff_v0_1.md`](tip_67a_ekyc_neutrality_opaque_challenge/tip_67a_kickoff_v0_1.md) - draft dispatch-ready kickoff (v0.6), [`tip_67a_closeout_v0_1.md`](tip_67a_ekyc_neutrality_opaque_challenge/tip_67a_closeout_v0_1.md) - closeout (v0.1); baseline `e8cf836`; BUILT + Contractor spot-check ACCEPT (171/171; +1 disclosed carried TIP-66 signedAt fix), ready for Codex commit; **eKYC neutrality refactor (shipped-code, incl. policy flag + error codes + wire serializer)** per `docs/adr/ekyc_neutrality_decision_v0_1.md` — reframe `TRANSACTION_BOUND_EKYC_PROFILE` → `CHALLENGE_BOUND_EKYC_PROFILE`; `bindingNonceHash`/`externalTransactionId`/`externalSessionId` → `Challenge` (a **truly opaque string, no `sha256:` requirement**) / `ClientReference` / correlation, **echoed (exact surfaces pinned), not interpreted** (no "transaction" concept in the eKYC core); echo verbatim in response DTOs only, NOT in the hash graph (`manifestHash`/golden byte-identical); back-compat for the old profile value on BOTH wire AND persisted-row strings + old request field keys (`bindingNonceHash`/`externalTransactionId`, via a custom request converter — STJ can't multi-name) with a conflict rule (existing sessions/clients unbroken; legacy-row + old-field-keys tests); updates `signflow_integration_contract`/`lld_01`/`lld_03`. NO signing/verification-surface change (→ 67B). Output = the frozen opaque-challenge/profile contract (67B's input) |
| TIP-67B | `tip_67b_neutral_verifiable_proof/` | [`tip_67b_planning_brief_v0_1.md`](tip_67b_neutral_verifiable_proof/tip_67b_planning_brief_v0_1.md) - planning brief (v0.9), [`tip_67b_kickoff_v0_1.md`](tip_67b_neutral_verifiable_proof/tip_67b_kickoff_v0_1.md) - build kickoff (v0.4), [`tip_67b_closeout_v0_1.md`](tip_67b_neutral_verifiable_proof/tip_67b_closeout_v0_1.md) - build closeout (v0.2); baseline `72a7b41`; **BUILT + Contractor spot-check ACCEPT (2 rounds, 176/176), ready for Codex commit by allowlist (NOT pushed)**; post-spot-check patches: Fix A signature-byte-tamper test (isolates ECDSA `VerifyData==false`), Fix B `signflow_integration_contract` self-contained verifier algorithm; **neutral verifiable eKYC proof + verification view**, depends on TIP-67A — signs a neutral proof claim (`proofVersion=neutral-proof-v1`, package metadata, hashed `identityRef`, result, single `assuranceLevel`, sorted checks, ordered `evidenceEngines[]`, `signedAt`, signed `challenge`, `signedManifestHash`, `resultHash`), exposes dedicated verification view + sign-time `publicKeyJwk`/`publicKeyFingerprint`, and includes reference consumer verification with fail-closed trust-anchor matrix (forged-JWS+attacker-JWK, kid/alg mismatch, private-JWK field, mirror tamper, resultHash mismatch, challenge mismatch, rotate-key). Summary route unchanged; no document/transaction binding in TagEkyc; no runtime verifier endpoint; golden hashes byte-identical; prod HSM/KMS/JWKS/CA/RFC-3161/webhook = P0; decision-basis binding = P1 |
| TIP-68 | `tip_68_prod_hsm_signing/` | [`tip_68_planning_brief_v0_1.md`](tip_68_prod_hsm_signing/tip_68_planning_brief_v0_1.md) - planning brief (v0.8), [`tip_68_kickoff_v0_1.md`](tip_68_prod_hsm_signing/tip_68_kickoff_v0_1.md) - kickoff (v0.6; **authoritative DoD source-of-truth**), [`tip_68_closeout_v0_1.md`](tip_68_prod_hsm_signing/tip_68_closeout_v0_1.md) - closeout (v0.1); baseline `12f5302`; **BUILT + Contractor spot-check ACCEPT (2 rounds, 190/190, SoftHSM e2e 5/5 token-opened), ready for commit** — round-1 caught 9/9-green-hiding-zero-token-coverage (CRITICAL), forced real SoftHSM2 e2e, round-2 verified genuine (on-token CKM_ECDSA + reference-verified JWS + fail-closed self-test via real key mismatch + concurrent). Pre-dispatch review history: **deep-code-review round-3 = 4 P1 + 3 P2, all patched**: runtime signs only the neutral proof (no 2nd signature; "proof+manifest"=proof binds manifestHash); rotation trust-anchor history + notice (consumer keeps old kid/fingerprint pinned, never trust embedded JWK alone); PKCS#11 signature byte-equivalent to LocalDev `SignData(SHA256,IeeeP1363)`; **eager startup fail-closed** (not lazy DI resolve); shared construction + byte-conformance; forward-compat scope (verifier fail-closed / mapper tolerant-read); SoftHSM provisioning recipe. Earlier rounds: digest-rule pinned vs double-hash, JWKS deferred to A2, "production grade"→custody/integrity-origin, PIN process-readable, SoftHSM-CI = STOP/RRI, `KeyObjectId`≠`Kid`. **Re-confirm before dispatch**; **P0 prod-crypto Slice A** — production signing custody via an **HSM PKCS#11** adapter behind `IEvidenceSigner` (same ES256/JWS envelope; private key never leaves the token), dev/CI = **SoftHSM** on the same code path, **environment-gated fail-closed DI** (production MUST NOT silently use the in-process key), **key rotation** (historical packages verify via the TIP-67B persisted sign-time key — no JWKS), pinned `kid` scheme (`tagekyc-es256-<year>-v<n>`). Decision basis `docs/p0_prod_crypto_decomposition_and_decision_brief_v0_1.md` (D-P0-1 HSM; D-P0-2 = TSA/CA/qualified NOT required for evidentiary value per Luật GDĐT 20/2023 Điều 10/11 + NĐ 23/2025, DEFERRED). JWKS = Slice A2; CA migration kept additive (self-describing scheme + fail-closed-on-unknown). Golden `manifestBodyHash`/`packageHash`/`manifestHash` byte-identical (signing downstream); no hash-chain/claim-shape/neutrality change; LocalDev adapter retained; no runtime verifier engine. CA cert / RFC-3161 TSA / qualified non-repudiation / webhook signing = deferred (legal milestone / separate surface) |
| TIP-69 | `tip_69_verification_engine_slice1/` | [`tip_69_planning_brief_v0_1.md`](tip_69_verification_engine_slice1/tip_69_planning_brief_v0_1.md) - brief (v0.7), ACCEPTED + committed; verification-engine slice 1 (Verification-Extension seam) — NFC chip-evidence decomposition: canonical `NfcEvidenceDecisionBasis` → server-owned `PayloadHash` (`HashCanonical("tip-69-nfc-evidence-decision-basis", basis)`) → manifest/proof (binding surface = PayloadHash, NOT ReasonCodes/SanitizedSummaryRef); server-authoritative fail-closed (`NFC_EVIDENCE_DECOMPOSITION_REQUIRED` / `NFC_PAYLOAD_HASH_MISMATCH` / ExternalPreStaged→ReviewRequired+`DIRECT_CLIENT_UPLOAD_UNTRUSTED`); serverDecisionResult normalized before hashing; `CAPTURE_BOUND_TO_SESSION` server-stamped iff `IsCaptureBindingTrusted`; `IFaceMatchEngine` port + deterministic fixture (seam only); no manifest/proof SHAPE change, `CalculateAssuranceLevel` unchanged, legacy golden byte-identical; 5 GPT+Codex rounds + adversarial CODE verify; committed `e1d9e29` + completion-level test `448a05e` + sanitized-label hardening `d6e1f9b`; baseline `140ea29` |
| TIP-70 | `tip_70_face_match_evidence/` | [`tip_70_planning_brief_v0_1.md`](tip_70_face_match_evidence/tip_70_planning_brief_v0_1.md) - brief (v0.4), ACCEPTED + committed; verification-engine slice 2 — FaceMatch evidence path (mirrors TIP-69): `FaceMatchEvidenceDecisionBasis` → server-owned `PayloadHash` (`HashCanonical("tip-70-face-match-decision-basis", basis)`) → manifest/proof; **reference-source LOCKED to `ChipDg2FromTrustedNfc` bound to a same-session `Passed` NfcValidation** (TIP-69 append-time fail-closed = the guarantee; flags not re-read) whose input artifact is the referenced NfcReadArtifact — stops image-similarity-as-identity (`FACE_MATCH_REFERENCE_*`); server-owned `isMatch`/threshold (0.80 const); `FACE_MATCH_BELOW_THRESHOLD` / `FACE_MATCH_DECISION_BASIS_MISMATCH` / `FACE_MATCH_PAYLOAD_HASH_MISMATCH`; role-aware live-binding; score adapter-declared, `IFaceMatchEngine` NOT wired (real engine = TIP-71); no SHAPE/assurance change, legacy golden byte-identical; 3 GPT+Codex rounds + **mutation-tested** adversarial CODE verify; committed `5626e9d` (+ `d6e1f9b` sanitized-label hardening); baseline `448a05e` |
| TIP-71 | `tip_71_agent_side_facematch/` (+ agent repo `TagEkyc.CaptureAgent`) | [`tip_71_planning_brief_v0_1.md`](tip_71_agent_side_facematch/tip_71_planning_brief_v0_1.md) - brief (v0.5), **BUILT + adversarial-CODE-verified; engine-verified on web photos, NOT production-ready**; **client CaptureAgent reference (NOT a server change)** — agent (own git repo, sibling solution, Contracts-DTO-only, no ViewFaceCore dep in server) runs a REAL ViewFaceCore/SeetaFace6 1:1 face-match (decode→detect→landmark **pts5/Light**→extract→compare) + submits NFC `Passed` + FaceMatch evidence via the EXISTING TIP-69/70 contract (server unchanged); earned-engineName (fixture path = `fixture-facematch`); engine-failure fail-closed; no-raw agent-boundary controls (in-memory/zeroed/no persist/log/dump); challengeHash byte-identical to server (independent canonicalizer + golden). Server-side: `lld_03` challengeHash formula+golden + `Tip05` golden+PcAgent-happy-path (no server semantic change). **Adversarial verify caught: marker-not-real-engine (remediated) + pts68-misaligns-recognizer (fixed→pts5; gap 0.39); loop misdiagnosed as threshold-debt 2×.** DEBT (NOT production-ready): threshold operating-point calibration (0.80 conservative vs SeetaFace-native 0.62 → TIP-70 follow-on), engine-adequacy on real DG2-low-res-vs-selfie + dlib comparison, license-packet completeness, cross-principal binding. ArcFace/InsightFace pretrained OFF-LIMITS (non-commercial dataset trap); dlib = fallback. Native inference opt-in (Homeowner-run local). Baseline `d6e1f9b` |
| TIP-72 | `tip_72_liveness_evidence/` | [`tip_72_planning_brief_v0_1.md`](tip_72_liveness_evidence/tip_72_planning_brief_v0_1.md) - brief (v0.5), **BUILT + adversarial-CODE-verified; server-side Liveness evidence path only**; mirrors TIP-70 for Liveness: additive `LivenessEvidenceDecisionBasis` -> server-owned `PayloadHash` (`HashCanonical("tip-72-liveness-decision-basis", basis)`) -> manifest/proof; server-derived `isLive` from score >= 0.80, adapter verdict audit-only; `Passed` requires trusted same-session live media binding (`LivenessMedia` preferred, exact session-bound `SelfieImage` fallback); below-threshold/verdict-conflict/unbound media coerce `ReviewRequired`; malformed/bare/hash-mismatch fail closed; earned-method rule rejects `silent-face` in this slice and accepts only `fixture-liveness` + `passive-2d-only`; no server PAD inference/ONNX/decode, no manifest/proof shape change, no `CalculateAssuranceLevel` change; passive OSS is not certified PAD and the assurance-policy debt remains. Real Silent-Face/MiniFAS engine = TIP-73 agent-engine. Baseline `62f9904` |
| TIP-73 | `tip_73_agent_side_liveness/` (+ agent repo `TagEkyc.CaptureAgent`) | [`tip_73_planning_brief_v0_1.md`](tip_73_agent_side_liveness/tip_73_planning_brief_v0_1.md) - brief (v0.3), **BUILT + real-ONNX orientation smoke PASS; agent-side Silent-Face/MiniFASNet-V2 liveness engine**; server delta is only the earned-`silent-face` allowlist consistency check (`silent-face-minifasnet` + `minifasnet-v2+onnxruntime-1.20.1`, grade `passive-2d-only`; fixture cross-cases fail closed, no new error codes); engine identity proof-bound via `evidenceEngines -> resultHash`/ES256-JWS, not the liveness basis `PayloadHash` or manifest hash; CaptureAgent adds fixture + real liveness source, canonical 2.7x face crop, BGR `/255` NCHW, softmax index 0 live, fail-closed/no-raw disposal, LivenessMedia-only mapping, license/provenance packet, deterministic tests, and opt-in real-ONNX smoke. Local model/media are gitignored and excluded from commits; passive OSS liveness is not certified PAD/ISO-30107 and threshold calibration remains follow-on. Baselines: server `2cd9c2d`, agent `4f8fa6a` |
