...
$baseline = @(Import-Csv -LiteralPath (Join-Path $docDir 'a3_broker_claim_baseline_trx_inventory.tsv') -Delimiter "`t")
$census = @(Import-Csv -LiteralPath (Join-Path $docDir 'a3_broker_claim_failed_run_census.tsv') -Delimiter "`t")
$before = @{}
...
foreach ($repo in @('server', 'agent')) {
    foreach ($file in @(Get-ChildItem -LiteralPath $roots[$repo] -Recurse -File -Filter '*.trx' -ErrorAction Stop)) {
        ...
        $after[$path] = [pscustomobject]@{ file = $file; sha = $sha }
    }
}
...
foreach ($path in $new) {
    ...
    if ([IO.Path]::GetFileName($path) -notmatch '^a3-broker-claim-[A-Za-z0-9._-]+\.trx$') {
        $outside += $path
    }
}
...
foreach ($row in $census) {
    ...
    foreach ($field in @('total','passed','failed')) {
        if ([int]$row.$field -ne [int]$c.$field) { throw "FAILED_COUNTER_MISMATCH $field $($row.path)" }
    }
    if ([int]$row.skipped -ne [int]$c.notExecuted) { throw "FAILED_SKIP_MISMATCH $($row.path)" }
