# A3 v0.6 — proposed mutation inventory and source freeze
...
| A | M | `src/TagEkyc.CaptureAgent.Core/CaptureAgentProductionConfigGate.cs` | `E4A6EC64CB48740A774FE9A7B05FB0E105CDA344B3F1FCF40F0A0D9456CABB70` | AR | F01 / §5.1 / A3_ManagedRetentionGate_RequiresServerAuthority |
| A | M | `src/TagEkyc.CaptureAgent.Core/CaptureRuntimeAgentCoordinator.cs` | `73EBE5B435649EE5E903F7404181F885969B2DA41CB2D5F3B3036B8770D70FC5` | AR | F02 / §5.3 / A3_ReceiptClosure_SixMetadataPlusTwoPublishedSources |
...
- No .gitattributes, git configuration, blanket source-file LF canonicalization, lockfile or dependency-version mutation is authorized. CP §10.1 is the required runtime SQL-only representation transform inside the single new A3 migration: CRLF→LF→CRLF for every multiline executed payload, CRLF→LF only for body-drift comparison. It is not a source-freeze exception. Raw hashes are full 64-character comparisons. At later separately authorized landing, export exact indexes and check both Git blob/working representations and text-diff closure; no automatic old-or-new SHA acceptance.
...
- Current docs-only v0.6 output is this inventory, parent, Agent/result and checkpoint successors plus the review ledger. E01 and broker companions are consumed unchanged at their v0.5 exact SHA; all seven v0.5 artifacts are preserved. No source file from this table was changed by drafting.

The two added Agent M rows only close F01/F02. `CaptureRuntimeHttpClient.cs`, Host/WPF composition, ports/orchestrator and the two Agent A3 tests were already rows and now also own their §5 gate/receipt joins. `WindowsCaptureRuntimeJournal.cs` is not a mutation target: its six-operation capacity/schema/whitelist semantics remain unchanged. The existing N row `tests/TagEkyc.IntegrationTests/Tip88C1C6BA3MigrationTests.cs` and A3 runner own CP §10.1 F03 proof; no extra project/tool/dependency path is introduced.
...
## 5. Agent read-only proof dependencies

These two exact Agent files are read-only consumed sources for F02's unchanged six-operation journal and record shape. They are NOT M rows and must remain unchanged; no journal schema/capacity expansion is proposed.

| Agent-relative read-only path | Raw SHA-256 |
| --- | --- |
| `src/TagEkyc.CaptureAgent.Client/WindowsCaptureRuntimeJournal.cs` | `204C945F032C0AFAD1D533F58438CFCE89ED62D0B772BA5F0974438417C7DBFF` |
| `src/TagEkyc.CaptureAgent.Core/CaptureRuntimeAgentPorts.cs` | `212F5FF3C8DC7644E7CCCF40C3E4382A8174E49A97BD17E36554A742B86F200C` |

CP10.1's exact required proof is `A3_RetentionMigration_CheckoutRepresentationsConverge`, in the existing N-row `Tip88C1C6BA3MigrationTests.cs`; the existing A3 runner owns its independent checkout/index build and execution orchestration. No new mutation path follows.

## Changelog

- v0.6: add exactly the two missing Agent M paths for F01/F02 and join the migration representation proof to existing migration/test/runner rows. No product mutation or generic source-hash normalization.
