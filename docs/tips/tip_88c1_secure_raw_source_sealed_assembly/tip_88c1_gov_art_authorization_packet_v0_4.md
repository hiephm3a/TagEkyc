# TIP-88C1 GOV/ART Authorization Packet v0.4

**File:** `docs/tips/tip_88c1_secure_raw_source_sealed_assembly/tip_88c1_gov_art_authorization_packet_v0_4.md`
...
| Baseline | `d1f0aba06ca72bd16a4d9b3d381ad91db1ac2bb8` |
| v0.4 packet path | `docs/tips/tip_88c1_secure_raw_source_sealed_assembly/tip_88c1_gov_art_authorization_packet_v0_4.md` |
| Ratification-record path | `docs/tips/tip_88c1_secure_raw_source_sealed_assembly/tip_88c1_gov_art_authorization_packet_ratification_v0_3.md` |
...
v0.4 supersedes v0.3, v0.2, and v0.1 for future review or ratification. Superseded
...
The git commit chain is normative for this ledger. Each successor record binds
the exact `PreviousRatificationRecordSHA256` and
...
record's containing-commit ancestry. A predecessor that was never separately
committed may be recorded as `UNCOMMITTED_PRE_RATIFICATION`, but neither that
record nor a chain relying on it is activation-eligible. A later committed
successor begins the activation-eligible chain by binding the exact committed
predecessor.
...
| Pre-authorized expected fixture fault | A real integrity failure must never green a deliberate negative-test cell. | Only an exact immutable pre-operation match can use the PASS-capable reviewer branch. | No fault injection is authorized. |
...
| Mutable/self-referential ratification | New version per transition, predecessor commit binding, and external final-record Git verification. | Append-only authorization history without self-reference. | Record remains inactive. |
...
| Required field | C1 v0.4 value |
| --- | --- |
| Packet id/name | `TIP-88C1-GOV-ART-FIXTURE-EVIDENCE-v0.4` |
...
| Least-privilege access | Ingress exact create/write; reconciler exact fenced provisional inspect/read/commit/abort; assembly exact committed read; lifecycle exact object/residue delete/hold; fixture evidence identity exact bucket create/whole-bucket enumerate/empty-bucket delete only; no other listing. | Production IAM/key authority and Gate B `ART-007`. |
...
7. CASes missed debt to `CleanupDebtOverdue`, records a hard-cap breach, keeps
...
`FaultOperationStartedAtUtc`. A quarantine matches only when all six fields,
the observed integrity-fault type, and the current run/object/packet bindings
are exact. Authorization is consumed once and cannot be replayed or broadened.
...
| Bucket delete and fixture expiry | `BucketDeleteDeadlineUtc < FixtureExpiresAtUtc` | The hard cap is at most run start plus 90 minutes; an in-run object is created no earlier than run start and expires 24 hours later. |
...
Coverage is 13/13 required relation families. In particular, the bucket-delete
...
| `ExpectedFaultType` | Exact pre-authorized integrity-fault kind expected from one deliberate test operation | One test/run/object; closed Build-Brief test taxonomy | Expected-fault and observed-fault evidence | `FixtureEvidenceReviewer` selects from reviewed taxonomy | Expected-fault record | `FIXTURE_EXPECTED_FAULT_TYPE_INDETERMINATE` |
| `ExpectedFixtureFault` | Immutable single-use authorization for one deliberate negative-test integrity fault | Exact test/run/object/type/packet before operation | Expected-fault repository and operation audit | `FixtureEvidenceReviewer` creates | Expected-fault record and consumption evidence | `FIXTURE_EXPECTED_FAULT_INDETERMINATE` |
...
| `MaximumEvidenceRunDuration` | Constant 30 minutes | Every Gate A evidence run in v0.4 | Reviewer from packet/run derivation | Packet amendment only | Packet and run row derivation version | `FIXTURE_DURATION_INDETERMINATE` |
...
- complete 13-row deadline-relation coverage with no unsatisfiable pair;
...
Create ratification record v0.3 bound to this packet SHA-256, record v0.2
SHA-256, and the explicit inactive predecessor-commit sentinel, then return
both for independent review. Do not request Build Brief, implementation,
evidence execution, or ratification.
