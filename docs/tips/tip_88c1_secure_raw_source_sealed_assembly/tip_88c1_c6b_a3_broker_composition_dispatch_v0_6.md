...
**Version:** v0.6 — executable contract catalogue candidate
**Date:** 2026-09-13
...
**Predecessor:** `tip_88c1_c6b_a3_broker_composition_dispatch_v0_5.md`, SHA `9A0254769139577D78F0FD6374D1E052ECE4B3D500BFAD5FBB7337531F7FD38F`, preserved byte-for-byte together with all five companions and its review ledger. This bounded successor corrects external F01/F02/F03 only; no business/consent decision is reopened.
...
| Agent production retention gate | LANDED / BLOCKS RETAINED PATH | `CaptureAgentProductionConfigGate.cs` rejects RawVault before Managed execution; AR §5.1 pins the bounded Managed exception with independent current server authority, not a local enablement default |
| A2 journal/receipt close | LANDED / SIX METADATA SLOTS ONLY | `CaptureRuntimeAgentCoordinator.cs` currently compares the entire receipt to six slots; AR §5.3 requires separate metadata/raw partitions without changing the six-entry journal |
...
| `tip_88c1_c6b_a3_retention_checkpoint_contract_v0_6.md` | Capability/binding/snapshot lineage, complete/R3/R4/R5 dispatch, C1/C3 export compatibility | `EC42987AA5F7A8FEA3642C60C23A80D57033645870E4D98DE7B62728E41CB21D` |
...
| `tip_88c1_c6b_a3_agent_result_contract_v0_6.md` | Canonical A1 amendment, public result projection, acceptance/completion callers, Agent streaming/receipt | `D97F33F688F88CAF21F0B8C7EE9384A31D1F11C5C6142804A34530D9056C9B74` |
| `tip_88c1_c6b_a3_mutation_inventory_v0_6.md` | Exact proposed future files, raw-source SHA and ownership/proof join | `E2B4EB58C1D0CE0C71BDE1033B4E103C458E45F255BA60C967144D5010FF325D` |

E01 and broker companions remain v0.5 byte-exact and are adopted unchanged by this v0.6 catalogue at the hashes above. Their v0.5 owner labels identify origin, not an independent or competing authority. AR, CP and inventory are v0.6 successors; only their bounded corrections replace the corresponding v0.5 surfaces.
...
A2 PendingBind, HandoffAccepted, DelegationAttempted and R22-first recovery remain unchanged. AR §5.3 changes only receipt validation for retained mode: six metadata journal operations remain six; two published raw results are receipt-only. The new raw sender does not reacquire device execution authority. No Site-A/Site-B ownership model is introduced.
...
| --- | --- | --- |
| A3.0 | Exact reference/retention/lineage forward schema, canonical A1 amendments and fresh checkpoint resolver | Apply/Down/reapply; CP10.1 independent CRLF/LF build/migration convergence and non-EOL drift RED; denied/withdrawn/expiry and role negative matrix |
...
| A3.3 | R25 acceptance, Managed gate, retained Agent sender/lease/six+two receipt and Client completion join | Two real class uploads through A3.2 + no pre-R1 read; six-only A2 and six+two retained closure, malformed/missing raw receipt RED, unchanged six-entry journal; non-retained and independent export regression |
...
Full-source-boundary review of v0.5 is recorded in its preserved review ledger and the external reviews. This v0.6 cycle rechecks F01/F02/F03, their invalidated caller/callee/proof/allowlist joins and a blocker-only sentinel sweep; it does not reset closed consent/architecture decisions. Census lanes do not count as review. From round3 record accumulated non-convergence causes; round5 root-cause checkpoint and round10 hard stop remain enabled. Stop at a real missing authority/evidence dependency, not after a fixed quota of findings.
...
- Managed RawVault request must reach real synthetic retained path only with current server configuration and exact retained binding; restoring old unconditional gate makes its positive proof RED. Legacy/VerifyAndDiscard behavior stays independent.
- Receipt closure must prove six metadata journal entries and eight receipt entries for retained success, never a raw journal operation/digest or slot-count-derived mode. Missing/duplicate raw slot and restart revival controls must fail closed.
- Every multiline A3 migration SQL payload uses CP10.1 execution normalization; independent checkout/index builds and PostgreSQL observers prove complete Up/Down/Up representation convergence, with real non-EOL/lone-CR drift rejected.
...
The following inherited evidence is checked against live bytes for v0.6; values describe raw working representation, not assumed Git LF exports. This is a census inventory, **not complete future implementation allowlist**. Predecessor versions and unrelated work remain untouched.
...
S01/S05 business semantics remain CLOSED. This v0.6 candidate corrects the three external v0.5 findings at the Managed gate, coordinator receipt/journal join and migration representation boundary. Source census is not proof execution. The non-normative v0.6 review ledger records bounded independent review and exact provenance; final external review and Homeowner implementation grant remain required.
...
Preserve every predecessor, including all seven v0.5 artifacts and v0.4 exact bytes. No new consent question or separate TIP/RRI is opened by this draft.


## Changelog

- v0.6: bounded external F01/F02/F03 correction only: add two Agent M paths, pin six-journal/eight-receipt closure and all-migration SQL representation/negative controls. E01/broker v0.5 remain unchanged, consent/export/lifecycle boundaries closed, no implementation authority.
