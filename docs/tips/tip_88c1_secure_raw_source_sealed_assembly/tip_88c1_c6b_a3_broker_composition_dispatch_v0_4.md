...
**Version:** v0.4 — reuse existing consent; bounded reference integration
**Date:** 2026-09-13
...
**Predecessor:** `tip_88c1_c6b_a3_broker_composition_dispatch_v0_3.md`, SHA `F18885D92750139F0A1C5D3F375020772A20E1CA6CAB8CDF41548C5E21AFFD7D`, preserved. This successor replaces v0.3 operative consent-input wording; older files remain historical.
...
## 3. E01 — reuse existing consent; integrate its reference before completion

**Business decision CLOSED. Technical classification: CONSENT_REFERENCE_INTEGRATION.** The requirement to ask the Homeowner where to obtain a new retention consent is withdrawn. A3 must not create a second consent purpose/form merely because its permit has purpose SourceRetention.

### 3.1 What is reused, and what the repository still lacks

| Existing fact | Repository evidence | Integration consequence |
| --- | --- | --- |
| Personal-data processing consent already covers the approved use | Homeowner clarification in §1 | Reuse the existing consent and provenance; no new subject action |
| B2 subject-export consent append | `20260720022629_Tip88B2SubjectExportConsent.cs:673-755` | Export event creation remains after Completed; do not call it early |
| Latest E3 replacement | `20260724015546_Tip88B1E3ResolverReadBoundary.cs:34-120` | Completed gate and SubjectRawBiometricExport purpose remain unchanged |
| B2 already models consent provenance | `RawExportSubjectConsent.cs`: ConsentTextVersion, ConsentTextContentHash, ExternalConsentArtifactRef | Reuse reference/provenance concepts; no new signature/canonicalization scheme |
| Session/capability contracts lack this reference | `BusinessConsumerContracts.cs`; `CaptureRuntimeExecutionContracts.cs`; `CaptureCapabilityPersistenceRequest` | Add the minimum server-side reference binding; do not claim this caller/field already landed |
| Passed capture and Client authentication | Existing evidence/context types | Neither silently creates a consent record; bind the actual existing consent reference |

The absence of a TagEkyc pre-completion reference field does **not** establish that the subject has not consented. It establishes missing integration into this path. No external consent database was inspected, and no upstream storage/API is claimed to exist in this repo.

### 3.2 Bounded technical binding proposal

The application-level input is a reference to the existing consent, not a request for a new consent. Resolve it in the authenticated BusinessConsumer/session boundary before the retention decision and capability are created.

- Proposed internal `ExistingConsentReference`: opaque `ExternalConsentArtifactRef` plus the source's immutable record/version identity. These are not assumed UUIDs or numeric revisions and must not be fabricated from session ID.
- Proposed internal `ResolvedExistingConsent`: source reference/version, exact authenticated Client and subject/session association, existing consent provenance, current effective/withdrawn status and applicable validity. The resolver verifies the relation against the actual consent source or its existing authenticated evidence; a caller string alone is not verified evidence.
- No new consent-text wording, special SourceRetention checkbox, new subject signature or new consent collection event is required. SourceRetention is the **permit's** purpose; do not require the old consent record to have been authored using this new literal.
- Persist a local immutable binding ID/revision linking that resolved reference to PrincipalId + ClientApplicationId + VerificationSessionId + the server-selected retention policy/class set. This is a provenance link, not a second subject agreement and not an export permit. It does not mandate a separate consent aggregate/table; place the link in the minimal reviewed authority storage.
- Both authority paths may refer to the same underlying ExternalConsentArtifactRef. Retention must not borrow the Completed-gated B2 event as a prerequisite; later B2/export still uses its unchanged authorization checks.
- The existing consent owner's withdrawal/update must invalidate the applicable retained-authority projection through a deterministic integration path. Do not synthesize a second user withdrawal workflow. Runtime missing/mismatched/withdrawn/expired evidence fails closed; “business reuse approved” is not a runtime allow-all.
- CaptureAgent receives neither this Client consent reference nor PrincipalId/API key as an authority input; capability/binding convey only the already-ratified Agent contract.

Exact binding storage, resolver implementation/caller, withdrawal synchronization, locks and field limits belong to the remaining A3 implementation-contract work. They must be frozen against actual source facts, not invented as already-landed functions. A3 can continue drafting those interfaces without asking the Homeowner to approve consent again. This successor does not add a generic consent framework, new trust provider or standalone consent TIP.

### 3.3 Reuse proof obligations

`ExistingConsentReuse_NoSecondSubjectAction`: the same seeded **consent source record**, resolved through the actual integration boundary, permits pre-completion retention and remains the provenance reference for later independently authorized export. No new subject agreement, signature, consent-collection action or UI interaction is required. Existing authorized B2 Granted/Withdrawn projections and required provenance/authority audit remain intact; this proof asserts shared underlying reference, not zero consent-database events. Seeding synthetic source consent is valid setup; pre-seeding the retention/export decision being tested is not.

`ExistingConsentReference_IsBoundNotAssumed`: valid reference positive control; wrong Client/session/subject/version, missing record, withdrawal and expiry each fail at the actual reference/authority check. Mutation “any nonempty reference means consent” must turn RED. Do not use an unrelated API authentication failure to claim this proof.

## 4. Retention authority and immutable lineage contract

### 4.1 Separate durable authority class

Proposed logical records are named `SourceRetentionDecision` and `SourceRetentionPermit`; these are **new, not aliases** for export tables. Grant creation is server-owned after E01 existing-consent reference resolution, before capability issuance, never by calling AuthorizeExportAsync.
...
| ConsentEvidenceId / ConsentEvidenceRevision | UUID / Int64 of the local immutable E01 reference binding | Nonempty/positive; refers to existing source consent/version, not a newly requested consent or B2 export permit |
...
This is a field/authority contract, **not complete DDL**. E01's reference-binding schema/FK/resolver must close before literal new tables/functions/migration/ACL can be frozen. Existing export authorization tables, their FKs, purpose checks and public response semantics are not repurposed.
...
Consent withdrawal/expiry before fresh R1 must serialize with its exact validation. After R1, retain committed state and use existing checkpoint outcomes; never infer “delete” from an expired HTTP request. Finite lock/CAS details for the existing-consent reference binding remain part of E01-dependent DDL, not a license to skip serialization.
...
1. A3.0: materialize E01 existing-consent reference resolution/binding and withdrawal synchronization; freeze retention DDL/function/lock/ACL and immutable principal/authority fingerprint; finalize S02/S03 canonical amendments and exact allowlist.
...
The following v0.2 evidence remains unchanged and was rehashed for v0.4; values describe raw working representation, not assumed Git LF exports. This is a census inventory, **not complete future implementation allowlist**. Predecessor versions and unrelated work remain untouched.
...
- E01 business reuse: CLOSED BY HOMEOWNER. Existing-consent reference integration remains Builder-owned technical work; no new consent workflow or repeat Homeowner consent question.
...
No separate TIP/RRI is opened. Continue technical A3 drafting using the existing consent as the approved input; finish exact reference-binding/DDL/ACL/allowlist/canonical projections and obtain CC/GPT review on exact bytes before requesting implementation authority. Do not halt merely to ask for a separate subject consent already rejected by the Homeowner.
