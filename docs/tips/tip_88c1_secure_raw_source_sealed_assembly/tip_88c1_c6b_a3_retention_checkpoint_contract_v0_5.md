# A3 v0.5 — retention snapshot and custody checkpoint contract

Status: candidate implementation contract, documentation only; not implementation authority. Consumes the E01 reference/permit contract in the same v0.5 catalogue. No new subject agreement, no early export authorization, no change to the export `Completed` guard. Existing migrations are evidence, never the mutation targets.
