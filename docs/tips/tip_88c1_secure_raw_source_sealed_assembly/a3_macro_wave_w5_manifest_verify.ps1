...
$manifestPath = Join-Path $doc 'a3_macro_wave_w5_candidate_manifest_v1.tsv'
$sidecarPath = "$manifestPath.sha256"
$excluded = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
foreach ($name in @(
    'a3_macro_wave_w5_candidate_manifest_v1.tsv',
    'a3_macro_wave_w5_candidate_manifest_v1.tsv.sha256',
    'a3_macro_wave_w5_review_packet_v1.md'
)) {
...
$w5Trx = @($paths.Keys | Where-Object { $_ -match '/a3-w5[^/]*\.trx$' })
$w5DirectoryTrx = @($paths.Keys | Where-Object { $_ -match '(?i)/TestResults/a3-w5/[^/]+\.trx$' })
$outsideNameFamily = @($w5DirectoryTrx | Where-Object { $_ -notin $w5Trx })
if ($outsideNameFamily.Count -gt 0) {
    throw "W5_OUTSIDE_NAME_FAMILY_TRX count=$($outsideNameFamily.Count) paths=$($outsideNameFamily -join ',')"
}
$failed = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
foreach ($key in $w5Trx) {
    if (-not $manifestIndex.Contains($key)) { throw "W5_TRX_NOT_IN_MANIFEST path=$key" }
    $xml = [xml][IO.File]::ReadAllText($paths[$key])
    if ([int]$xml.TestRun.ResultSummary.Counters.failed -gt 0) { [void]$failed.Add($key) }
}
$census = @(Import-Csv -LiteralPath (Join-Path $doc 'a3_macro_wave_w5_failed_run_census_v1.tsv') -Delimiter "`t")
$censusIndex = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
foreach ($entry in $census) {
    if (-not $censusIndex.Add($entry.path)) { throw "W5_FAILED_CENSUS_DUPLICATE path=$($entry.path)" }
    if (-not $failed.Contains($entry.path)) { throw "W5_CENSUS_NOT_FAILED path=$($entry.path)" }
    if (-not $manifestIndex.Contains($entry.path)) { throw "W5_FAILED_TRX_NOT_IN_MANIFEST path=$($entry.path)" }
    $fileSha = (Get-FileHash -LiteralPath $paths[$entry.path] -Algorithm SHA256).Hash
    if ($entry.sha256 -cne $fileSha) { throw "W5_FAILED_TRX_SHA_MISMATCH path=$($entry.path)" }
}
foreach ($key in $failed) {
    if (-not $censusIndex.Contains($key)) { throw "W5_FAILED_TRX_UNCLASSIFIED path=$key" }
}

$inventory = @(Import-Csv -LiteralPath (Join-Path $doc 'a3_macro_wave_w5_trx_inventory_v1.tsv') -Delimiter "`t")
$inventoryIndex = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
foreach ($entry in $inventory) {
    if (-not $inventoryIndex.Add($entry.path)) { throw "W5_TRX_INVENTORY_DUPLICATE path=$($entry.path)" }
    if (-not $manifestIndex.Contains($entry.path)) { throw "W5_TRX_INVENTORY_NOT_IN_MANIFEST path=$($entry.path)" }
    $fileSha = (Get-FileHash -LiteralPath $paths[$entry.path] -Algorithm SHA256).Hash
    if ($entry.sha256 -cne $fileSha) { throw "W5_TRX_INVENTORY_SHA_MISMATCH path=$($entry.path)" }
    ...
    if ([int]$entry.total -ne [int]$c.total -or [int]$entry.passed -ne [int]$c.passed -or
        ...
        throw "W5_TRX_INVENTORY_COUNTER_MISMATCH path=$($entry.path)"
    }
}
foreach ($key in $w5Trx) {
    if (-not $inventoryIndex.Contains($key)) { throw "W5_TRX_OMITTED_FROM_INVENTORY path=$key" }
}

$predecessor = @(Import-Csv -LiteralPath (Join-Path $doc 'a3_macro_wave_w4_candidate_manifest_v1.tsv') -Delimiter "`t")
$predecessorIndex = @{}
...
$allowed = @(Import-Csv -LiteralPath (Join-Path $doc 'a3_macro_wave_w5_allowed_delta_v1.tsv') -Delimiter "`t")
$allowedIndex = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
foreach ($entry in $allowed) {
    if (-not $allowedIndex.Add("$($entry.change)`t$($entry.path)")) { throw "W5_ALLOWED_DELTA_DUPLICATE path=$($entry.path)" }
    if ([string]::IsNullOrWhiteSpace($entry.reason)) { throw "W5_ALLOWED_DELTA_REASON_MISSING path=$($entry.path)" }
}
foreach ($entry in $observed) { if (-not $allowedIndex.Contains($entry)) { throw "W5_UNDECLARED_DELTA change_path=$entry" } }
foreach ($entry in $allowedIndex) { if (-not $observed.Contains($entry)) { throw "W5_ALLOWED_DELTA_NOT_OBSERVED change_path=$entry" } }
...
"manifest_sha=$sha"
"w5_trx_in_manifest=$($w5Trx.Count)/$($w5Trx.Count)"
"outside_name_family_failed=0"
"w5_trx_inventory=$($inventory.Count)/$($w5Trx.Count)"
"observed_delta=$($observed.Count) allowed_delta=$($allowed.Count) outside_allowlist=0"
"w5_failed_runs=$($failed.Count)"
