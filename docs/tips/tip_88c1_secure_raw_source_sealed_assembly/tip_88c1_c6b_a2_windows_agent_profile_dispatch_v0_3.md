...
**Version:** v0.3 — review candidate, not implementation authority  
**Date:** 2026-09-12  
...
**Predecessor:** `tip_88c1_c6b_a2_windows_agent_profile_dispatch_v0_2.md`, preserved byte-exact; SHA-256 `5AEABE0DD328962BB92890FD095B27D5460C28B5E3A0432459315E343A748615`.  
...
binding/horizon → durably flush binding result → zero secret buffers → flush
binding-confirmation ACK → durable DelegationAttempted claim (§6) → existing
submit-only orchestrator with runtime client
→ cleanup. Failed ACK cannot start a second execution on exact redelivery.
...
slot timeout 120s or envelope expiry, whichever earlier. The 5s limit bounds
header/body intake, not server-binding completion. Persist the original slot
absolute deadline with BindPending; retry/restart does not extend it. A timeout
never becomes a success ACK; terminalize/clear memory without inventing binding.
...
DelegationAttempted=false, allocated and flushed with that identity.
...
ONLY THEN return HTTP200 Cache-Control:no-store, Connection:close,
body {"accepted":true}, flushing before dispose. This is a BINDING-CONFIRMATION
ACK, not a capture/evidence completion ACK. Capture/device delegation follows
durable binding confirmation and successful ACK; never manufacture completion.
Before the FIRST device/preview invocation, durably set DelegationAttempted=true
under the same coordinator/journal lock. A false value may pass that gate once
after current binding/horizon checks and ACK; true can never reacquire devices.
...
Until successful ACK the sender may retain and re-deliver the SAME envelope.
Restart with BindPending first calls R22 using persisted capability/operation.
An exact committed binding is recovered and durably flushed; clear re-delivered
secret bytes before ACK. If R22's typed unavailable result cannot recover a
...
If local binding was flushed but ACK failed, exact redelivery may return the
same nonsecret ACK after validating/recovering the binding/horizon, with no
second R21 operation or second delegated execution. Never-started work can pass
the durable start gate once; started-but-unresolved work cannot. Do not discard this recovery
identity solely because the ACK transport failed.
...
| Confirmed binding / before local flush | No ACK; R22 restores result, flush before ACK |
| Local binding flush / ACK transport loss | Same tuple may receive repeated ACK, no new binding/execution |
| ACK success / before start-claim flush | Binding recovered, claim false permits first delegation through the same gate |
...
erase unresolved nonsecret lineage. At that deadline, even without a BindingId,
flush Disposition=Terminal with the existing identity, deadlines, known binding
if any and DelegationAttempted as one bounded payload-free uncertainty tombstone.
No late ACK, capture or revival of that attempt; a late R22 result is informational
...
already-started execution/submission state, not an excuse to reacquire devices.
...
| Bind ACK loss / restart | §6 R22-first; exact redelivery may recover same pending R21; confirmed binding flush and zeroization precede repeated ACK; no second execution |
...
| P05 CeremonyTests.Bind_RestartReconcile_UsesNonsecretJournal | All §6 crash points; restart R22-first + exact redelivery; same BindingId/operation, no secret on disk | ACK before confirmed binding, new BindOperationId, R21 without redelivered secret, wrong-envelope recovery, premature device acquisition |
...
| P08 HandoffTests.DualLoopback_OneSlot_BoundedAndZeroed | Real dual sockets, binding-confirmation ACK, all §6 crash/redelivery cases; max-size positive control | early ACK, second execution on repeated ACK, mismatched recovery, wildcard, port preemption, slowloris, secret log/cache |
| P09 CompositionTests.HostAndWpf_ReachOneCoordinator | Actual CLI/WPF STA caller; crash before/after durable start claim and before first append; at most one device invocation | missing start claim, restart reacquisition, dead registration, separate owner, device start before bind |
...
| P12 CeremonyTests.SecretLeaks_AllTerminalPathsClear | All §6 crash points plus success: synthetic sentinel absent from stdout/log/disk, buffers zero before ACK/device | secret persistence, skipped clearing, ACK before binding flush, capture before confirmation |
