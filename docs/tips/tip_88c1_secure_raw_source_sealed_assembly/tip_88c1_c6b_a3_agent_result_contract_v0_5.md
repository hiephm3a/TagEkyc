# A3 v0.5 — public result, acceptance and Agent integration contract

**Date:** 2026-09-13
**Status:** executable-contract proposal; no implementation authority.
**Owner:** A3 parent v0.5. Existing A1/A2 code is the baseline, not evidence of A3 execution.
...
Receipt retains existing six R24/R25 slots and adds only the two already-named RawChipDg2Portrait/RawLiveSelfieImage slots on durable S results. `submitted_all` for retained mode requires those two source slots as well as its metadata slots. ResumePending -> existing `submission_state_unknown` status plus NextAction `WAIT_FOR_SOURCE_RECONCILIATION`, not fake raw slots or an export-ready claim. VerifyAndDiscard's receipt remains unchanged. This receipt contains no consent reference supplied as new authority, broker details, raw/key bytes or export permit.
