...
**Version:** v0.8 successor candidate  
**Supersedes for implementation:** ratified v0.7 SHA-256 `2DB31ED6EFD55EC57166D19F43130EC21D87524FC5309C92434194B9C4219EDD`; implementation remains paused until this exact v0.8 successor is reviewed and ratified  
...
Homeowner ratifies this exact v0.8 SHA; this candidate does not self-authorize
...
```text
request_fingerprint_as_predecessor := R13 fingerprint with generation pair (G, G+1)
request_fingerprint_as_successor   := R13 fingerprint with generation pair (G-1, G)
```
...
pair differs. Underflow/overflow or a non-positive derived generation is 400
before lookup. Neither digest construction uses database state, role selection,
target existence or branch information.

Inside the atomic function, a locked `Predecessor` branch selects and returns
only `p_request_fingerprint_as_predecessor`. A locked `Successor` branch accepts
only when `p_request_fingerprint_as_successor` equals the exact durable
completion fingerprint, then selects and returns that value. The returned
...
ratification of this exact v0.8 SHA:
...
| 38 | `RawIngressAuthSuccess_HandsUnreadBodyToA3ExactlyOnce` | IntegrationTests `Tip88C1C6BA1RawIngressBoundaryTests.cs` | HTTP27 | YES | proposed; authorized only after exact-v0.8 ratification |
| 39 | `RawIngressAuthDenial_DoesNotReadBodyOrInvokeA3` | IntegrationTests same | HTTP27 | YES | proposed; authorized only after exact-v0.8 ratification |
...
proposed for ratification with this exact v0.8 successor. It records the
