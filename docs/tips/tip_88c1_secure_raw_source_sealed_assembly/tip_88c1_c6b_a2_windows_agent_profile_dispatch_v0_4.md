...
**Version:** v0.4 — review candidate, not implementation authority  
**Date:** 2026-09-13  
...
**Predecessor:** `tip_88c1_c6b_a2_windows_agent_profile_dispatch_v0_3.md`, preserved byte-exact; SHA-256 `439B9B2714BB87C400C1F78772966651424E65CC763464E121686CCF0E65D8DA`.  
...
| Configuration cache/polling | Core/RawExportIngressContracts.cs:37–185 | Untracked working candidate, NOT landed; reuse synchronization/freshness pattern; remove numeric revision rollback rejection, adapt full A1 response and disabled-state handling |
...
(opt-in ProjectReference to ../../../TagEkyc.CaptureAgent/src/TagEkyc.CaptureAgent.Client/TagEkyc.CaptureAgent.Client.csproj)
...
there is no product reverse dependency or reference cycle. The exact MSBuild
property EnableCaptureAgentA2Acceptance defaults to false. When absent/false,
exclude the new acceptance .cs file from Compile and do not load the sibling
ProjectReference: ordinary TagEkyc.sln build remains standalone. When true,
include exactly that reference and acceptance file; missing sibling project
causes an explicit build error before tests, never a silent skip. No server
product project receives an Agent reference. §8 pins both graph proofs.
Reuse existing
...
| Bind | capability ID, BindOperationId, nonsecret local session ref and verification-session observation, exact confirmed BindingId/horizon if known | No secret on disk; flush confirmed binding, erase secret-bearing buffers, then durably set HandoffAccepted before receipt/device; unaccepted restart is R22-first; accepted restart uses the one durable delegation CAS (§6) |
...
TAG-EKYC-CRT1; uppercase method; exact path; CredentialId-N-lowercase; generation decimal;
...
The credential header and signed CredentialId field both use Guid.ToString("N")
ordinal lowercase, exactly matching the server parser; uppercase is rejected.
...
Validate CaptureAgentId against enrollment, nonempty ConfigurationId and positive
...
for business revision. An authenticated R23 200 selects the exact server-assigned
(ConfigurationId, ConfigurationRevision); replace cache after full validation,
even A/5 → A/3 or A/10 → B/1. Numeric revision is NOT a client anti-rollback
counter: remove the working candidate's lower-revision-is-STALE comparison.
The same exact tuple may also return different effective content: landed R15 can
change ConfigurationOverrideId and R23 merges that override into the 15 fields
without returning its identity (Foundation:1270–1281,1714–1738). Accept that fully
validated authenticated 200 as well; do NOT call the effective projection an
immutable catalog revision. No new field or client override resolver is added.
ETag remains opaque. 304 requires an existing same-enrolled-lineage document,
the request using that document's ETag, an exactly consistent returned ETag and
full current freshness recheck; it never advances EffectiveAt/ExpiresAt. Reconnect fetch runs
...
flush nonsecret BindPending → R21 (restart: R22-first recovery per §6) → confirm
binding/horizon → durably flush binding result → zero secret buffers → durably
flush HandoffAccepted=true → emit/replay HTTP receipt (delivery is not authority)
→ durable DelegationAttempted CAS (§6) → existing
submit-only orchestrator with runtime client
→ cleanup. Receipt loss does not revoke durable acceptance or require sender
redelivery to continue; exact redelivery cannot start a second execution.
...
capability expiry, never extend it; restart has no secret and uses R22 first,
then only exact redelivery may recover uncommitted R21 under §6. P12's
...
slot timeout 120s or envelope expiry, whichever earlier, bounds the pending
handoff before durable acceptance. The 5s limit bounds header/body intake, not
server-binding completion. Persist the original slot deadline with BindPending;
retry/restart never extends it. Timeout before acceptance terminalizes pending
intake without inventing binding/acceptance. After acceptance, receipt write loss
or listener timeout does not undo acceptance; execution still obeys its frozen
ExecutionExpiresAtUtc and existing readiness checks, never a renewed horizon.
...
envelope atomically claims the empty slot. Allocate ONE UUIDv4 BindOperationId;
durably flush SignFlowLocalSessionRef, VerificationSessionId, CaptureCapabilityId,
BindOperationId, original envelope expiry and slot deadline before R21.
PendingBind also has nonsecret Disposition=Pending/Bound/Terminal and
HandoffAccepted=false and DelegationAttempted=false, allocated/flushed with that
identity. These are nonsecret booleans, not remote-delivery evidence.
Keep secret only in mutable process memory. Perform R21 with that exact tuple.
On ambiguity, only live retry with the SAME business body/BindOperationId and
fresh CRT1, or R22 with the SAME capability/operation, is permitted.
After exact server binding confirmation, durably flush BindingId,
ExecutionExpiresAtUtc, frozen returned revisions and Disposition=Bound; clear capability lease,
serialized bind request and all secret-bearing scratch/staging buffers.
Under the coordinator/journal lock, ONLY AFTER those secret buffers are cleared,
durably transition HandoffAccepted=false→true. It means the exact server-bound
handoff/result is durably local, capability material has been cleared and this
Agent accepts responsibility for at most one execution. Do not persist a claim
that the remote sender received any response.
Only then emit HTTP200 Cache-Control:no-store, Connection:close,
body {"accepted":true}, flushing before connection disposal. This ACK is a
replayable RECEIPT of durable local acceptance, not capture/evidence completion.
Remote receipt delivery is NOT execution authority and is never a prerequisite
for restart continuation. A failed response write must not undo acceptance.
Before the FIRST device/preview invocation, under the same coordinator/journal
lock durably CAS DelegationAttempted=false→true with HandoffAccepted=true and
current binding/horizon/readiness checks as prerequisites. Only that CAS winner
may invoke devices; false acceptance or true attempted can never acquire them.
Restart with true but no confirmed completion returns RECOVERY_REQUIRED; the
marker is an at-most-once claim, not proof that capture ran or completed. A crash
after this flush but before invocation sacrifices that attempt, not safety.

Until receiving a receipt the sender may retain and re-deliver the SAME envelope.
Restart with HandoffAccepted=false first calls R22 using persisted capability/operation.
An exact committed binding is recovered and durably flushed; clear re-delivered
secret bytes, then flush HandoffAccepted before receipt or delegation. If R22's
typed unavailable result cannot recover a
binding and the exact envelope is re-delivered within original expiry/deadline,
R21 may run with the SAME stored BindOperationId, never a new identity.
Transport/auth/dependency failure is not proof of absence and cannot trigger
blind new work. Without exact secret redelivery and without recovered binding,
return RECOVERY_REQUIRED. Never persist or reconstruct the secret.

Exact redelivery must match all nonsecret envelope identity fields; server R21
verifies the presented secret if a bind is needed. A different session/capability/
local reference/expiry is BUSY/rejected and cannot overwrite pending identity.
A recovery-only slot may reopen explicitly for that stored tuple, never an
unrelated envelope; no automatic permanent listener or new execution.
If binding was flushed but HandoffAccepted is false, reconcile/revalidate before
finishing secret cleanup and durable acceptance; no capture before that boundary.
With HandoffAccepted=true, restart may continue through the single delegation
CAS without any sender redelivery or knowledge of receipt delivery. Existing
capture-context/readiness and execution-horizon checks still apply; missing
context fails closed, never fabricated from the journal.
Exact redelivery receives the same nonsecret receipt after validating the stored
accepted tuple/horizon and clearing new secret-bearing intake buffers. It creates
no second R21, BindingId, acceptance or delegation transition. Different envelopes
remain BUSY/rejected. Never-started accepted work can pass the start gate once;
started-but-unresolved work cannot. Do not discard this recovery identity solely
because the receipt transport failed.

| Fault boundary | Permissible recovery / residue |
| --- | --- |
| Before BindPending flush | No ACK and no R21; exact redelivery can begin |
| After BindPending flush, before R21 | No ACK; restart R22-first, then exact redelivery may complete same pending R21 |
| R21 ambiguous / before commit | No ACK; live bounded memory retry; restart R22-first with exact redelivery if needed |
| Server commit / response loss | No ACK; R22 recovers exact committed binding, no second operation |
| Confirmed binding / before local result or HandoffAccepted flush | Acceptance false; no capture; reconcile exact binding, flush result, clear secret, then accept |
| HandoffAccepted flush / before first response byte | Accepted=true, attempted=false; restart may win one delegation CAS without redelivery |
| Response write fails after acceptance | Same durable state and continuation as preceding row; exact redelivery replays receipt |
| HTTP200 reaches sender / crash before delegation CAS | Same accepted=true/attempted=false state by design; same one-CAS continuation |
| Concurrent exact redelivery / receipt replay | Same receipt, no second R21, BindingId or device acquisition |
| Start-claim flush / before invocation or first append | Claim true: RECOVERY_REQUIRED on restart, no second device acquisition |
| Different redelivery during pending recovery | BUSY/rejected, pending journal unchanged |

Terminal parse error/timeout/cancel/shutdown closes slot and both listeners and
zeroes app-owned secret buffers. Before original slot/capability deadline, do not
erase unresolved nonsecret lineage. For HandoffAccepted=false, at that deadline,
even without a BindingId,
flush Disposition=Terminal with the existing identity, deadlines, known binding
if any, HandoffAccepted and DelegationAttempted as one bounded payload-free
uncertainty tombstone.
No late acceptance/ACK, capture or revival of that terminal pending attempt;
a late R22 result is informational
only, not permission to delegate. This is NOT a claim that no server commit occurred.
An explicit end of the terminal local attempt may retire it before an explicitly
requested new execution using normal server authority and a fresh envelope. Never
retarget/reissue the old capability or operation as recovery; no automatic new
capability/session generation. Keep at most one terminal tombstone, compact under
the journal bound; pending unexpired work still cannot be evicted for capacity.
P05/P08/P09/P12 cover every listed crash point, unknown-binding terminal retirement
and original-horizon rejection. The §5 execution-horizon retirement remains for
durably accepted execution/submission state, not an excuse to reacquire devices.
Receipt loss alone is not cancellation, expiry or terminal retirement.
...
| Bind receipt loss / restart | §6 acceptance false: R22-first recovery; acceptance true: receipt delivery irrelevant, one durable delegation CAS; exact redelivery replays receipt, no second execution |
...
| P04 WireTests.Configuration_FullShapeFreshnessAndIdentity | Real 15-field response; valid A/5→A/3, A/10→B/1 and same tuple with changed override projection accepted; disabled retained as state; exact ETag/304 | wrong Agent, expired/malformed, six-field/camelCase, 304 extends horizon, numeric-only or same-pair-content rejection |
| P05 CeremonyTests.Bind_RestartReconcile_UsesNonsecretJournal | All §6 crash points; unaccepted R22-first recovery; accepted restart without redelivery; same BindingId/operation | receipt before HandoffAccepted, device with acceptance false, second BindOperationId, secret persistence, wrong-envelope recovery |
...
| P08 HandoffTests.DualLoopback_OneSlot_BoundedAndZeroed | Real dual sockets, replayable acceptance receipt; crash before response bytes and response write failure; concurrent exact redelivery | early receipt, second execution, receipt delivery required to resume, mismatched recovery, wildcard, port preemption, slowloris, secret log/cache |
| P09 CompositionTests.HostAndWpf_ReachOneCoordinator | Actual CLI/WPF STA caller; accepted restart after lost/delivered receipt, crash before/after delegation CAS and first append | absent acceptance/start claim, two CAS winners, restart reacquisition, device before acceptance |
...
| P12 CeremonyTests.SecretLeaks_AllTerminalPathsClear | All §6 crash points plus success: sentinel absent stdout/log/disk, buffers cleared before HandoffAccepted | secret persistence, acceptance before zeroization, capture with acceptance false, missing delegation CAS |

P05/P08/P09/P12 independently exercise before/after HandoffAccepted flush,
before any response byte, response-write failure, delivered receipt followed by
crash before delegation CAS, and concurrent exact receipt redelivery. Positive
controls prove accepted=false gives device count 0; accepted=true/attempted=false
permits exactly one durable CAS winner; accepted=true/attempted=true gives restart
acquisition count 0. Remove/bypass either durable guard to make these proofs RED.
No test treats a local flush as evidence of remote delivery.
...
tools/Invoke-CaptureAgentA2Acceptance.ps1. Before container/test execution, runner
verifies exact sibling repository, frozen baseline/consumed-source preconditions
and Agent.Client project. Runner restores the opt-in graph with
-p:EnableCaptureAgentA2Acceptance=true before the --no-restore test command;
normal server build/test does not set the property. Runner executes: dotnet test
...
direction. A2 completion requires ordinary server solution build with property
absent/false AND opt-in acceptance build/test with property true. Fake transport tests complement these real boundary proofs.
P13 runner proof AcceptanceGraph_DefaultStandalone_OptInRequired is owned by
Invoke-CaptureAgentA2Acceptance.ps1 (build-level proof, not a fake test PASS).
Use an isolated checkout without sibling Agent, not moving/deleting a user's repo:
default TagEkyc.sln build succeeds; opt-in fails explicitly before test execution.
Restoring the exact sibling enables real acceptance. No silent missing-project skip.
...
6. Full Agent Windows x64 build/tests plus exact A1 acceptance fixture; both
   ...
   Also require §8 default standalone server build and opt-in cross-repo proof.
...
| G3 Cross-repo real-server proof | Existing server IntegrationTests/Postgres/TestServer | Opt-in test-only reference and exact test file/runner, §§2/8; standalone server default; no product reverse edge |
...
Changelog v0.3: external F01 binding-confirmation ACK/recovery replaces intake ACK;
F02 exact server configuration assignment replaces numeric anti-rollback, including
landed override projections (the external same-pair immutability subclause is not
applied because it contradicts A1 SQL). F01 crash propagation also pins a nonsecret
start claim and terminal pending-intake retirement, without changing A1;
F03 opt-in acceptance graph preserves standalone server build; CC lowercase
CredentialId pin is explicit. Sections 1–8,10–12 propagated; §9 hashes unchanged.
No product implementation or executed-test claim. READY_FOR_EXTERNAL_A2_REVIEW.


Changelog v0.4: F04 makes HTTP200 a replayable receipt of durable HandoffAccepted,
not a remote-delivery prerequisite. Propagated journal, ordering, accepted/pending
retirement and P05/P08/P09/P12. Predecessor v0.3 stays byte-exact. No new API,
A1/A3/A4 decision or implementation authority. READY_FOR_EXTERNAL_A2_REVIEW.
