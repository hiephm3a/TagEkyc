...
**Version:** v0.12 — CRT1 proof completion and consumed-source reconciliation  
...
**Split checkpoint authority:** non-executable umbrella v0.5 SHA `F41C24BD0BD58F6B070EFE17BB69E7F89823F37B94F48A339870CD19D67825FD`
...
| CRT1 exact signed-bytes micro-RRI | `EDA8703C3969F6A7043E496553FEC3E2B3AD72462C319EE10BE3491490489903` |
| CRT1 proof-owner project-boundary correction | `A79601F3944DAD71F12FCEABEE6B6BE6E1DACAC88C4E134BECF44B3D5519FFB5` |
| Literal DDL master | `86B4B9960F943A86B6178FA548945A99D90057D61D4ED631151D447D844113D7` |
| Operation/function master | `B1E3F320C32691756DACEF5A206B6D0303E811377B7304623C43C56ECBEAC277` |
| Transition SQL 01 | `C22555381F7B0FED33FE7483E5CDE30A2C20000E3667D567729CFDAFF3649BC0` |
| Transition SQL 02 | `D7F99DA31CCCB4DFE5196B6A8AB9A4D9ADAA6B56EC63F745EE63AD0CFDDE8471` |
| Transition SQL 03 | `2A67E5D6ED533D4778D7B0A074366B08A29ECD3750ED94461F2CD1C5BD7729A4` |
| Transition SQL 04 | `EEBA46C7E328A04C0B5FEBBA91DA5F8D4BC01B41A27B634DB7CCDC7BB21DCF02` |
| PostgreSQL 16 core mutation evidence | `C508F055A1D15C427FD8A91012562AA5193F90362558C61B4D855DC24E4541E8` |
| PostgreSQL 16 runtime evidence | `B9DF7BF95F23210872E657A56425457230B42D668C3628216848BC1E61420945` |
| PostgreSQL 16 identity/audit evidence | `5C9DE1AB4C915C1090E532FAE84C36A3856D753161C7FD6DEAC22D9D05D03FD0` |
| PostgreSQL 16 expiry-on-denied evidence | `22DE86FDE294BD2A5CF13FB13E3A20BD0DA3265E237E106040490DB268FC4B19` |
...
JSON rejects unknown/duplicate members and trailing bytes. Mutation idempotency
exists only in one lower-case UUIDv4-N `Idempotency-Key` header, never duplicated
in body. Default JSON ceiling is 16 KiB; configuration publication is 32 KiB;
...
| POST runtime rotation complete | predecessor CRT1 + successor ROTATE1; atomic current-pointer cutover; bounded same-route successor/predecessor recovery after response loss; no reconcile route |
...
| existing Client cancel | terminalizes ActiveUnbound capability in same finalization transaction |
...
All request records live in `TagEkyc.Contracts.CaptureRuntime`; UUID wire form
is lower-case `N`, revisions are positive Int64, UTC is canonical `O` ending Z.
`Idempotency-Key` is header-only. “B” below means one transaction containing
...
| R13 `POST /api/ekyc/capture-runtime/credential-rotations/{rotationId}/complete` | first call: predecessor CRT1 + successor ROTATE1; exact RotationId/CandidateKeyId/SPKI/thumbprint. CredentialId is installation-stable; successor Generation is predecessor+1; CandidateKeyId is globally unique per rotation and remains in every identical fingerprint. After ambiguous response: (1) retry identical R13 once with successor CRT1; success returns the exact committed result; (2) if generically denied, retry once with the original predecessor CRT1 + successor ROTATE1 envelope; success performs the not-yet-committed cutover; (3) if both deny, terminate recovery and require fresh operator-authorized bootstrap enrollment. Exactly these attempts; no polling, configuration-role probe or reconcile route | N commit, then B 10→20→30→60; first-call dual PoP/current CAS atomically activates successor, marks predecessor Rotated, advances current pointer and writes operation/event; successor replay reads only the exact committed operation and cannot mutate; every denial is the same non-enumerating result |
...
| R26 existing `POST /api/ekyc/verification-sessions/{id}/cancel` | existing Client session owner → existing terminal result | existing finalization B 70→80; session+capability terminal state/events together; replay |
...
| R28 A1 startup route selection | deployment process + schema/ACL/sentinel/A3-port facts → one immutable route set or startup failure | read-only before listener accepts traffic; Prepared/mismatch/missing A3 fails startup; Activated maps canonical set only |
...
- R13 path carries RotationId only. The body is exactly CandidateKeyId UUIDv4-N,
  ...
  covers path RotationId, stable CredentialId/current Generation, all four body
...
- R20 path supplies VerificationSessionId. Body is the closed union
  ...
  replay returns 409 `EXISTING_MATCH_SECRET_UNAVAILABLE` with nonsecret ID,
  state, revision and horizon; ClientApplicationId derives from session auth.
- R21 body is exactly CaptureCapabilityId, CaptureCapabilitySecret and
  ...
  base64url characters decoding to 32 bytes; its keyed digest, never plaintext,
  enters the operation fingerprint. Success returns only BindingId,
...
Generation, CandidateKeyId, successor SPKI/thumbprint and Idempotency-Key. First, one
successor-CRT1 replay may read only an exact committed operation matching
RotationId, stable CredentialId, successor Generation, CandidateKeyId,
SPKI/thumbprint and fingerprint. If generically denied, one original-envelope
retry may cut over only while the predecessor is still Active/current and the
authorization is live. If that is also generically denied, recovery ends in
fresh operator-authorized bootstrap enrollment. No polling or state-disclosing
error is allowed. Before commit only predecessor CRT1 plus successor ROTATE1 may
authorize the cutover; after commit the predecessor has state `Rotated` and has
no authentication exception.

For R13 only, after ordinary successor CRT1 cryptographic/lifecycle/current-
generation authentication and committed nonce transaction N, exact committed-
...
| runtime-authenticated TagEkyc routes | `ICaptureRuntimeDbContextFactory.CreateAsync`, mandatory named `CaptureRuntimeOnline` secret reference; A1 tests use `tagekyc_c6ba_test_online_login` | authenticator+application roles; verifier/nonce then capability/bind/config/capture/evidence functions | no operator functions, ordinary Client repositories or direct table DML |
...
| offline provisioner product code | same operator factory contract, credential supplied only by A4 at deployment | R01/R02 code path and cutover invocation contract; no embedded secret | no online/runtime connection or Client API key |
...
| migration owner | existing `tagekyc_raw_export_deployer` | owns DDL/SECURITY DEFINER functions only | not an application LOGIN |
...
| successor verifier | A2 private; A1 receives SPKI/thumbprint/CandidateKeyId+dual proof | stable CredentialId + Generation+1; Pending→Active generation/current pointer atomically; same-R13 bounded successor replay then original-envelope retry | never instruct deletion on ambiguous transport result; no server private key; both generic denials terminate in fresh bootstrap enrollment |
...
src/TagEkyc.Infrastructure/CaptureRuntime/CaptureRuntimePersistenceBoundary.cs
...
tests/TagEkyc.ContractTests/Tip88C1C6BA1ProtocolTests.cs
tests/TagEkyc.IntegrationTests/Tip88C1C6BA1PersistenceTests.cs
tests/TagEkyc.IntegrationTests/Tip88C1C6BA1ConcurrencyTests.cs
...
| --- | --- | --- | --- |
| M | `src/TagEkyc.Api/Program.cs` | `ADC8176B12AF9D7FE4F36A32C040483E2B143E85ED93AB6D8DE7C24702D6E193` | DI/route selection |
...
| ?? | `src/TagEkyc.Api/RawExportSourceIngressEndpoints.cs` | `1023A127F4A5183C2A9A1D63060C5B7638EC66DE35F87CE084961EFC1F34598F` | unread-body seam |
...
Each row is one exact test method name. Every method invokes its callable
boundary and asserts response, durable residue and forbidden side effect.

| ID / exact method | Required RED mutation |
| --- | --- |
...
Migration apply/rollback/reapply, full server tests, callable ephemeral-role ACL
proof, secret scan and exact as-built packet PASS. This closes server foundation
only; it does not claim operational deployment or C6B-A completion.
