# TIP-04 API Key Authentication, Client Policy, and Session Lifecycle Kickoff v0.3

**File:** `docs/tips/tip_04_api_key_policy_session_lifecycle/tip_04_kickoff_v0_3.md`
**Version:** 0.3
**Status:** Dispatch-condition patches applied - ready for review
**Date:** 2026-06-10
...
### v0.3 - Dispatch-condition patches applied

- Resolved remaining implementation-affecting open questions for client policy shape, error/status convention, scope names, and review handoff.
- Pinned `Idempotency-Key` precedence: it is inert in TIP-04 and duplicate `externalSessionId` wins with `409 DUPLICATE_EXTERNAL_SESSION`.
- Pinned duplicate RequiredChecks behavior to reject duplicate check types with `INVALID_REQUIRED_CHECKS`.
- Pinned not-yet-completed GET defaults: `assuranceLevel = NONE`, `evidencePackageId = null`, `evidencePackageHash = null`, and `completedAt = null`.
- Clarified rejected authenticated create-session audit safe-marker boundary.
- Added dispatch precondition requiring git status reporting and confirmation that pre-existing dirty files remain out of TIP-04 implementation scope unless explicitly authorized.
...
- Duplicate `externalSessionId` validation takes precedence over `Idempotency-Key` in TIP-04.
- A repeated `POST /api/ekyc/verification-sessions` with the same `Idempotency-Key` and the same `externalSessionId` must return `409 DUPLICATE_EXTERNAL_SESSION`, not same-response replay.
...
- TIP-04 accepted business scopes are:
  - `business.session.create`
  - `business.session.read`
- TIP-04 must reserve, but not implement endpoint behavior for, these future scopes:
...
TIP-04 may add LocalDev/NonProduction runtime policy configuration and application-layer validation around the existing TIP-03 domain concepts. The default implementation direction is runtime policy options first.

Additive domain changes are allowed only if needed for clean application validation. TIP-04 must not add persistence-oriented policy schema, production policy versioning, production policy governance, or durable policy storage.
...
In TIP-04, not-yet-completed session summaries must use these defaults because TIP-06 is not implemented:
...
- Duplicate check types must be rejected with `INVALID_REQUIRED_CHECKS`.
...
Rejected authenticated create-session audit behavior:

- Rejected authenticated create-session audit is optional in TIP-04 and LocalDev-only unless a future audit policy says otherwise.
- If implemented, it may store only safe reason code, client application id, key prefix, request id, correlation id, endpoint/action name, and timestamp.
- It must not store raw request body, raw API key, raw headers, raw identity fields, raw artifacts, raw biometric/document/NFC/fingerprint payloads, plaintext sensitive payload, or raw binding nonce.
- Rejected request audit must not become an evidence or decision record.
...
- `401 Unauthorized` for missing, unknown, revoked, or expired API key.
- `401 Unauthorized` for disabled client when treated as an authentication failure.
- `403 Forbidden` for a valid authenticated key lacking scope.
- `403 Forbidden` for valid authenticated caller disallowed by client policy, including unauthorized purpose/profile/check, transaction-bound not allowed, or cross-client access.
- `404 Not Found` for missing session id.
- `409 Conflict` for duplicate `externalSessionId`.
- `422 Unprocessable Entity` must not be used in TIP-04 unless a later reviewed convention chooses it.
...
- RequiredChecks subset and empty behavior.
- Duplicate RequiredChecks check types are rejected with `INVALID_REQUIRED_CHECKS`.
...
- Not-yet-completed GET returns `assuranceLevel = NONE`, `evidencePackageId = null`, `evidencePackageHash = null`, and `completedAt = null`.
...
- Rejected authenticated create-session audit, if implemented, stores only safe reason/correlation/client markers and no raw body or sensitive payload.
...
- Duplicate `externalSessionId` wins over `Idempotency-Key`; repeated same-key/same-external-session create returns `409 DUPLICATE_EXTERNAL_SESSION`.
...
- No persistence-oriented policy schema or production policy versioning.
...
- Adding persistence-oriented policy schema, durable policy storage, or production policy versioning/governance.
...
- Letting `Idempotency-Key` override duplicate `externalSessionId` behavior.
- Implementing production or distributed `externalSessionId` uniqueness.
- Allowing duplicate `externalSessionId` per client in LocalDev storage after this v0.3 decision.
- Accepting `Required = false` checks as optional checks before TIP-05/TIP-06 defines optional evidence enforcement.
- Accepting duplicate RequiredChecks check types instead of rejecting them with `INVALID_REQUIRED_CHECKS`.
...
- Using `422 Unprocessable Entity` in TIP-04 without a later reviewed status convention.
- Returning `UNKNOWN` instead of `NONE` for not-yet-completed `assuranceLevel`.
...
Resolved by v0.2 and v0.3 review patches:
...
- Rejected authenticated create-session audit is optional LocalDev-only and must store only safe reason/correlation/client markers.
- Client policy shape uses LocalDev/NonProduction runtime policy options first; additive domain changes are allowed only if needed for clean application validation.
- Error status convention is pinned: `401` for auth failures, `403` for authenticated scope/policy/access denials, `400` for malformed/invalid field shape, `409` for duplicate `externalSessionId`, and no `422` in TIP-04.
- Scope names are accepted: `business.session.create` and `business.session.read`; future scopes are reserved as `capture.artifact.append`, `trusted.evidence.append`, `session.complete`, `webhook.retry`, and `admin.policy.manage`.
- After v0.3 review acceptance, implementation may start within the documented boundaries.

Remaining open questions:

- None that affect TIP-04 implementation dispatch.

## 19.1 Dispatch Preconditions

Before TIP-04 implementation starts, Codex must:

- report current `git status --short`;
- identify pre-existing dirty files;
- confirm pre-existing dirty files are not modified by TIP-04 implementation unless explicitly brought into scope by the user;
- STOP+ASK before editing any pre-existing dirty tracked file for TIP-04 implementation;
- keep implementation within the v0.3 kickoff boundaries after review acceptance.
...
| TIP04-V03-01 | Low | v0.2 still had implementation-affecting open questions and ambiguous edge behavior for idempotency precedence, duplicate RequiredChecks, not-yet-completed GET defaults, rejected-request audit, and dispatch preconditions. | Patched. v0.3 resolves dispatch-affecting open questions, pins edge behavior, updates STOP+ASK gates and tests, and adds dispatch preconditions. |
...
4. Are any temporary S1 shortcuts likely to become architectural debt? Yes, if allowed to escape local/dev boundaries: fake key comparison, inert `Idempotency-Key` handling, LocalDev duplicate `externalSessionId` rejection, runtime policy options, and any future expiration mutation. The v0.3 pinned decisions, naming/comment guardrail, and STOP+ASK gates keep them explicit.
...
```text
TIP-04 Kickoff v0.3 accepted for review.
