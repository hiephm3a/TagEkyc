# TIP-04 API Key Authentication, Client Policy, and Session Lifecycle Kickoff v0.1

**File:** `docs/tips/tip_04_api_key_policy_session_lifecycle/tip_04_kickoff_v0_1.md`
**Version:** 0.1
...
- Append-only audit events for:
  - API key authentication success/failure where appropriate and safe;
  - session creation accepted/rejected where appropriate;
...
- Mutating requests should accept `Idempotency-Key`, but TIP-04 may STOP+ASK before implementing durable idempotency if it would require a persistence decision.
...
- `externalSessionId` uniqueness may be enforced in local/dev storage if feasible. If reliable idempotency or uniqueness requires a durable store, STOP+ASK.
...
- Checks with `Required = false` should either be rejected in TIP-04 or explicitly treated as optional requested checks. The kickoff recommendation is to reject `Required = false` until optional checks have a clearer TIP-05/TIP-06 enforcement model.
...
- `expiresAt` is recorded but TIP-04 may avoid background expiration jobs.
- Read-time expiration handling may be planned, but if it mutates state it must append an audit event and remain local/dev only unless a durable store is approved.

Basic transition rules:

- Allowed in TIP-04:
  - no session -> `CREATED` on successful create.
  - `CREATED` -> `EXPIRED` only if a read/write path explicitly detects expiration and records an audited state change.
...
- session creation accepted;
- session creation rejected after authentication where recording is safe and useful;
- lifecycle state changes;
- access denied for cross-client session reads where recording is safe and useful.
...
- Adding background jobs or durable schedulers for expiration without a reviewed storage/runtime decision.
...
- Any idempotency or duplicate external session handling that is not backed by durable storage.
- Any read-time expiration state mutation.
...
1. API key transport: should S1 local/dev use `Authorization: Bearer <api-key>`, `X-TagEkyc-Api-Key`, or support both with one documented preference?
2. Idempotency: should TIP-04 implement in-memory idempotency for `POST /verification-sessions`, or only preserve request fields and defer reliable idempotency until durable storage is approved?
3. Duplicate `externalSessionId`: should local/dev reject duplicates per client, allow duplicates with warning, or defer uniqueness until durable persistence?
4. Optional checks: should `Required = false` be rejected in TIP-04, or should optional checks be preserved for later TIP-05/TIP-06 behavior?
5. Expiration: should GET mutate expired sessions to `EXPIRED` with an audit event, or should expiration remain a computed response concern until durable persistence exists?
6. Client policy shape: should TIP-04 add `allowedProfiles` and `allowsTransactionBoundProfile` to the TIP-03 domain model, or keep them in a runtime policy options model until a persistence design is approved?
7. Error status convention: should semantic policy violations use `403`, `400`, or `422` consistently?
8. Audit failures: which failed authenticated operations should append audit events in local/dev without overfitting production audit behavior?
9. Scope names: are the proposed scope strings acceptable, especially the reserved CaptureAgent and TrustedAdapter scope categories?
10. Review handoff: should the reviewer authorize implementation immediately after this kickoff, or require a patched v0.2 kickoff before dispatch?
...
4. Are any temporary S1 shortcuts likely to become architectural debt? Yes, if allowed to escape local/dev boundaries: fake key comparison, in-memory idempotency/uniqueness, and read-time expiration mutation. The patched naming/comment guardrail and STOP+ASK gates keep them explicit.
...
```text
TIP-04 Kickoff v0.1 accepted for review.
