# TIP-05 Capture Artifact and Evidence Result Recording Kickoff v0.3

**File:** `docs/tips/tip_05_capture_artifact_evidence_recording/tip_05_kickoff_v0_3.md`
**Version:** 0.3
**Status:** Additional review patches applied - ready for review
**Date:** 2026-06-11
...
### v0.3 - Additional deterministic dispatch patches

- Pinned deterministic error codes for cross-client/session write denial, artifact-not-found behavior, and fingerprint artifact rejection.
- Defined exact LocalDev/NonProduction policy fields for TIP-05 optional/supporting behavior: `allowedClientApplicationIds`, `allowedCaptureAgentIds`, `allowedOptionalEvidenceChecks`, and `allowedSupportingArtifactTypes`.
- Required capture artifact submissions to include stable non-raw hashes/references and required `ArtifactHash` for captured media/NFC/fingerprint artifacts.
- Required every accepted EvidenceResult to include `PayloadHash`.
- Added EvidenceResultType to allowed CaptureArtifactType compatibility rules.
- Clarified `SanitizedSummaryRef` boundaries.
- Updated tests, STOP+ASK gates, resolved decisions, and self-check history for subagent and GPT v0.3 findings.
...
- Cross-client capture/evidence writes return `403 SESSION_ACCESS_DENIED`.
...
Exact LocalDev/NonProduction TIP-05 policy fields:

- `allowedClientApplicationIds`: optional set of session-owning client application ids a CaptureAgent or TrustedAdapter credential may write for. If absent, the credential may write only sessions whose `clientApplicationId` equals the credential `clientApplicationId`.
- `allowedCaptureAgentIds`: optional set of request `CaptureAgentId` values accepted for a CaptureAgent credential. If absent, a present request `CaptureAgentId` must equal the caller `keyPrefix`. If request `CaptureAgentId` is omitted, TIP-05 stores the caller `keyPrefix` as the LocalDev capture agent identity.
- `allowedOptionalEvidenceChecks`: optional set of `RequiredCheckType` values that may accept optional evidence when not required by the session. If absent or empty, optional evidence is rejected.
- `allowedSupportingArtifactTypes`: optional set of `CaptureArtifactType` values that may be recorded as supporting artifacts when they do not directly map to a required or optional evidence check. If absent or empty, supporting artifacts are rejected.
...
- If `CaptureAgentId` is present in the request, it must equal the caller `keyPrefix` or appear in LocalDev `allowedCaptureAgentIds`.
- If `CaptureAgentId` is omitted, TIP-05 stores the caller `keyPrefix` as the LocalDev capture agent identity.
...
- Capture artifact submissions must include at least one stable non-raw reference: `ArtifactHash` or `MetadataHash`. TIP-05 has no `externalArtifactRef` field; introducing one requires a later reviewed contract patch.
- For `DocumentFrontImage`, `DocumentBackImage`, `SelfieImage`, `LivenessMedia`, `NfcReadArtifact`, and `FingerprintCapture`, `ArtifactHash` is required.
- `DeviceCaptureMetadata` may use `MetadataHash` without `ArtifactHash`.
- Missing stable hash/reference returns `400 INVALID_CAPTURE_ARTIFACT`.
- Invalid hash shape returns `400 INVALID_HASH_REF`.
- `ArtifactHash`, when required or provided, must be a hash reference shape such as `sha256:<value>`.
...
| --- | --- |
| `NfcReadArtifact` | Requires session `DocumentNfc` or LocalDev `allowedOptionalEvidenceChecks` containing `DocumentNfc`. |
| `SelfieImage` | Requires session `FaceMatch` or LocalDev `allowedOptionalEvidenceChecks` containing `FaceMatch`. |
| `LivenessMedia` | Requires session `Liveness` or LocalDev `allowedOptionalEvidenceChecks` containing `Liveness`. |
| `FingerprintCapture` | Requires session `Fingerprint` or LocalDev `allowedOptionalEvidenceChecks` containing `Fingerprint`. |
| `DocumentFrontImage` | Requires `DocumentOcr` required, `DocumentOcr` in `allowedOptionalEvidenceChecks`, or `DocumentFrontImage` in `allowedSupportingArtifactTypes`. |
| `DocumentBackImage` | Requires `DocumentOcr` required, `DocumentOcr` in `allowedOptionalEvidenceChecks`, or `DocumentBackImage` in `allowedSupportingArtifactTypes`. |
| `DeviceCaptureMetadata` | Allowed only when `DeviceCaptureMetadata` is in `allowedSupportingArtifactTypes` and the session has at least one RequiredCheck or at least one check in `allowedOptionalEvidenceChecks`. |

If the artifact type does not map to a required check, `allowedOptionalEvidenceChecks`, or `allowedSupportingArtifactTypes`, reject with `403 ARTIFACT_TYPE_NOT_ALLOWED`.

`FingerprintCapture` is allowed only when the session requires `Fingerprint` or LocalDev policy includes `Fingerprint` in `allowedOptionalEvidenceChecks`; otherwise reject with `403 FINGERPRINT_NOT_ENABLED`. `ARTIFACT_TYPE_NOT_ALLOWED` is reserved for non-fingerprint artifact type policy failures.
...
- Every accepted evidence result must include `PayloadHash`.
- `PayloadHash` must be a hash reference shape such as `sha256:<value>`.
- Missing or invalid `PayloadHash` returns `400 INVALID_HASH_REF`.
- `PayloadSignatureStatus` remains `PlaceholderUnverified`; TIP-05 must not implement production signature verification.
- `SanitizedSummaryRef` is optional. If present, it must be a sanitized logical summary reference, not a VaultRef, raw filesystem path, sensitive URL, raw artifact reference, raw payload reference, plaintext identity payload, or plaintext OCR/document/NFC/biometric/fingerprint value.
...
EvidenceResultType to input artifact compatibility:

| EvidenceResultTypeDto | Allowed input CaptureArtifactTypeDto |
| --- | --- |
| `CaptureQuality` | Any capture artifact type accepted for the target session. |
| `DocumentOcr` | `DocumentFrontImage`, `DocumentBackImage`. |
| `NfcValidation` | `NfcReadArtifact`. |
| `FaceMatch` | `SelfieImage`. `DocumentFrontImage` or `DocumentBackImage` only if that artifact type is in `allowedSupportingArtifactTypes`. |
| `Liveness` | `LivenessMedia`. `SelfieImage` only if `SelfieImage` is in `allowedSupportingArtifactTypes`. |
| `FingerprintMatch` | `FingerprintCapture`. |
| `FraudRisk` | Deferred in TIP-05. |

If an evidence result references an input artifact type incompatible with its `ResultType`, reject with `400 INVALID_EVIDENCE_RESULT`.
...
- If the mapped check is not required but appears in LocalDev `allowedOptionalEvidenceChecks`, accept it as optional evidence.
- If the mapped check is neither required nor listed in `allowedOptionalEvidenceChecks`, reject it with `CHECK_NOT_ALLOWED`.
- TIP-05 default is no optional evidence acceptance unless LocalDev `allowedOptionalEvidenceChecks` explicitly names the mapped check.
...
6. Cross-client access denial before capture/evidence write returns `403 SESSION_ACCESS_DENIED`.
...
Deterministic ownership/not-found rules:

- Cross-client capture/evidence writes always return `403 SESSION_ACCESS_DENIED`.
- Evidence input artifacts are resolved only within the target verification session. If an input artifact id is not found in that session's artifact list, return `404 CAPTURE_ARTIFACT_NOT_FOUND`.
- TIP-05 must not perform a global artifact lookup before same-session validation in a way that reveals another session's artifact existence.
- Disallowed `FingerprintCapture` returns `403 FINGERPRINT_NOT_ENABLED`.
- Other disallowed artifact type policy failures return `403 ARTIFACT_TYPE_NOT_ALLOWED`.
...
- Capture artifact request rejects missing stable hash/reference with `INVALID_CAPTURE_ARTIFACT`.
...
- Disallowed `FingerprintCapture` returns `FINGERPRINT_NOT_ENABLED`.
...
- Evidence result request requires `PayloadHash` and rejects missing/invalid `PayloadHash` with `INVALID_HASH_REF`.
- `SanitizedSummaryRef`, when present, rejects VaultRefs, raw paths, sensitive URLs, raw artifact refs, raw payload refs, and plaintext identity values.
...
- Evidence result input artifact types must be compatible with `EvidenceResultType`.
...
- Evidence result referencing an artifact not in the target session artifact list returns `404 CAPTURE_ARTIFACT_NOT_FOUND`.
- Evidence result with incompatible input artifact type returns `400 INVALID_EVIDENCE_RESULT`.
- Evidence result missing or invalid `PayloadHash` returns `400 INVALID_HASH_REF`.
...
- Disallowed fingerprint artifact returns `403 FINGERPRINT_NOT_ENABLED`.
...
- Adding optional/supporting policy behavior without using the v0.3 fields `allowedOptionalEvidenceChecks`, `allowedSupportingArtifactTypes`, `allowedCaptureAgentIds`, and `allowedClientApplicationIds`, or a later reviewed replacement.
- Accepting capture artifacts without `ArtifactHash` or `MetadataHash`.
- Accepting captured media/NFC/fingerprint artifacts without `ArtifactHash`.
...
- Accepting evidence results whose input artifact types are incompatible with `EvidenceResultType`.
- Accepting evidence results without `PayloadHash`.
- Treating `SanitizedSummaryRef` as a VaultRef, raw path, sensitive URL, raw artifact ref, raw payload ref, or plaintext identity payload.
...
- Cross-client write denials use `SESSION_ACCESS_DENIED`; missing same-session input artifacts use `CAPTURE_ARTIFACT_NOT_FOUND`.
- Disallowed `FingerprintCapture` uses `FINGERPRINT_NOT_ENABLED`; other artifact policy failures use `ARTIFACT_TYPE_NOT_ALLOWED`.
- Exact LocalDev policy fields are named: `allowedClientApplicationIds`, `allowedCaptureAgentIds`, `allowedOptionalEvidenceChecks`, and `allowedSupportingArtifactTypes`.
- Capture artifacts require stable non-raw hash/reference; captured media/NFC/fingerprint artifacts require `ArtifactHash`.
- Evidence results require `PayloadHash`.
- Evidence input artifact types must be compatible with `EvidenceResultType`.
...
### Subagent/GPT Additional Review Patch Report for v0.3

| Finding ID | Severity | Issue | Affected section | Recommended patch | Whether patch was applied |
| --- | --- | --- | --- | --- | --- |
| TIP05-V03-SUB-01 | Medium | Cross-client/session and input artifact ownership failures were not pinned to deterministic response codes. | Sections 6, 14, 16, 17 | Use `SESSION_ACCESS_DENIED` for cross-client writes and `CAPTURE_ARTIFACT_NOT_FOUND` for input ids not found in the target session. | Yes. |
| TIP05-V03-SUB-02 | Medium | Fingerprint artifact rejection could return either `FINGERPRINT_NOT_ENABLED` or `ARTIFACT_TYPE_NOT_ALLOWED`. | Sections 7, 14, 16, 17 | Use `FINGERPRINT_NOT_ENABLED` for disallowed fingerprint capture; reserve `ARTIFACT_TYPE_NOT_ALLOWED` for other artifact policy failures. | Yes. |
| TIP05-V03-SUB-03 | High | Optional/supporting authorization gates and CaptureAgent identity allowance lacked concrete policy fields. | Sections 6, 7, 9, 14, 16, 17 | Define exact LocalDev fields or move branches behind STOP+ASK. | Yes. Defined `allowedClientApplicationIds`, `allowedCaptureAgentIds`, `allowedOptionalEvidenceChecks`, and `allowedSupportingArtifactTypes`. |
| TIP05-V03-GPT-01 | High | CaptureArtifact records could be appended without stable hash/reference because `ArtifactHash` and `MetadataHash` were optional. | Sections 7, 16, 17, 19 | Require at least one stable non-raw hash/reference and require `ArtifactHash` for captured media/NFC/fingerprint artifacts. | Yes. |
| TIP05-V03-GPT-02 | High | Evidence results required same-session artifact ids but not artifact type compatibility. | Sections 8, 9, 16, 17 | Add EvidenceResultType to allowed CaptureArtifactType mapping and reject incompatible inputs. | Yes. |
| TIP05-V03-GPT-03 | High | EvidenceResult `PayloadHash` was optional, which weakens TIP-06 manifest readiness. | Sections 8, 16, 17, 19 | Require `PayloadHash` for every accepted EvidenceResult and validate hash shape. | Yes. |
| TIP05-V03-GPT-04 | Low | Supporting-document capture policy needed a named field. | Sections 6, 7, 19 | Name the LocalDev policy field instead of hardcoding implicit support. | Yes. Added `allowedSupportingArtifactTypes`. |
