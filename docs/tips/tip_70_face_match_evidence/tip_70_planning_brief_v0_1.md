...
5. **Honest recording** — method (`face-match`), `engineName/version`, `thresholdApplied`, and the **exact `matchScore`** live in the proof-bound basis (internal-confidential, NOT consumer-visible — exact score is fine since it never leaves the hashed internal payload); `verified: passed` must never imply liveness/PAD or certified grade. **The basis `sanitizedSummaryLabel` is sanitization-validated server-side** (same `IsSanitizedSummaryRefAllowed` rule as `request.SanitizedSummaryRef`) so no raw-PII/biometric/vault/url marker can enter the hashed payload. `CalculateAssuranceLevel` (`:556`) **unchanged**; `category=IdentityEvidence` recorded, not enforced.
