...
**Version:** 0.2
**Status:** ACCEPTED — FK patch verified (154/154). GPT GitHub review of `8454fbd` found a P1 the Tier-1 adversarial review missed (DoD #1 "FK" absent — migration had only PK+index+append-only triggers, no referential integrity). FK patch (2nd patch round) adds 6 FK constraints (5 child→`verification_sessions`, audit nullable, all ON DELETE Restrict; manifest→`evidence_packages` via nuance-A normalization of `EvidenceManifestRow.EvidencePackageId`/`ResultRef` to Guid) + a real orphan-reject test asserting Postgres `ForeignKeyViolation` SQLSTATE. Re-verified by reading code: 6 FKs present; orphan test genuine; regeneration regression-checked — all 6 append-only triggers + xmin concurrency token survived; old migration cleanly removed (single `20260621075836` migration). P2 hygiene (design-time creds, fixture container down) handled per builder report. Open: commit the FK patch (Persistence + IntegrationTests allowlist) — separate from the in-flight TIP-63 docs track.
**Date:** 2026-06-21
